...
public class EventFactory extends AbstractEventFactory {
	...
	/**
	 * The name of the topic of a "message transport ready" event triggered on an
	 * endpoint instance when it's message transport connection is ready for
	 ...
	 */
	public static final String TRANSPORT_CHANNEL_ADDED_TOPIC = "org/jitsi/videobridge/IceUdpTransportManager/"
			+ "TRANSPORT_CHANNEL_ADDED";

	/**
	 * The name of the topic of a "transport channel removed" event.
	 */
	public static final String TRANSPORT_CHANNEL_REMOVED_TOPIC = "org/jitsi/videobridge/IceUdpTransportManager/"
			+ "TRANSPORT_CHANNEL_REMOVED";
	/**
	 * The name of the topic of a "transport connected" event.
	 */
	public static final String TRANSPORT_CONNECTED_TOPIC = "org/jitsi/videobridge/IceUdpTransportManager/"
	...
	/**
	 * Creates a new "channel created" <tt>Event</tt>, which indicates the creation
	 * of a new COLIBRI channel.
	 * 
	...
	/**
	 * Creates a new "channel expired" <tt>Event</tt>, which indicates the expiry of
	 * a COLIBRI channel.
	 * 
	...
	/**
	 * Creates a new "conference expired" <tt>Event</tt>, which indicates the expiry
	 * of a COLIBRI conference.
	 * 
	...
	/**
	 * Creates a new "content created" <tt>Event</tt>, which indicates the creation
	 * of a new COLIBRI content.
	 * 
	...
	/**
	 * Creates a new "content expired" <tt>Event</tt>, which indicates the expiry of
	 * a COLIBRI content.
	 * 
	...
	/**
	 * Creates a new "endpoint created" <tt>Event</tt>, which indicates that a
	 * COLIBRI endpoint was created.
	 * 
	...
	public static Event endpointDisplayNameChanged(AbstractEndpoint endpoint) {
		return new Event(ENDPOINT_DISPLAY_NAME_CHANGED_TOPIC, makeProperties(endpoint));
	}

	/**
	 * Creates a new "message transport ready" <tt>Event</tt>, which means that the
	 * endpoint passed in {@link #EVENT_SOURCE} property has now it's message
	 * transport connection established and ready for sending/receiving data.
	 *
	...
	/**
	 * Creates a new "transport channel added" <tt>Event</tt>, which indicates that
	 * a COLIBRI channel was added to a Jitsi Videobridge TransportManager.
	 * 
	 ...
	 * that a COLIBRI channel was removed from a Jitsi Videobridge TransportManager.
	 * 
	...
	/**
	 * Creates a new "transport connected" <tt>Event</tt>, which indicates that a
	 * Jitsi Videobridge TransportManager has changed its state to connected.
	 * 
	...
	public static Event transportConnected(IceUdpTransportManager transportManager) {
		return new Event(TRANSPORT_CONNECTED_TOPIC, makeProperties(transportManager));
	}
	...
	public static Event transportCreated(IceUdpTransportManager transportManager) {
		return new Event(TRANSPORT_CREATED_TOPIC, makeProperties(transportManager));
	}

	/**
	 * Creates a new "transport manager state changed" <tt>Event</tt>, which
	 * indicates that a Jitsi Videobridge TransportManager has changed its state.
	 * 
	 * @param transportManager the changed transport manager object
	 *
	 * @return the <tt>Event</tt> which was created.
	 */
	public static Event transportStateChanged(IceUdpTransportManager transportManager, IceProcessingState oldState,
