...
public class PublicClearPortRedirectBundleActivator extends AbstractJettyBundleActivator {
	...
	protected boolean willStart(BundleContext bundleContext) throws Exception {
		...
		if (cfg.getProperty(PublicRESTBundleActivator.JETTY_PROPERTY_PREFIX + JETTY_TLS_PORT_PNAME) == null) {
			return false;
		}
	...
	protected Handler initializeHandlerList(BundleContext bundleContext, Server server) throws Exception {
		...
		handlers.add(new RedirectHandler(
				cfg.getInt(PublicRESTBundleActivator.JETTY_PROPERTY_PREFIX + JETTY_TLS_PORT_PNAME, 443)));
	 ...
	 * {@inheritDoc}
	 *
	 * Just skips few of the printed errors in case of not having permission to
	 * start it.
	 */
	@Override
	public void start(BundleContext bundleContext) throws Exception {
		try {
			...
			logger.warn("Could not start redirect from clear port(80) to secure port:" + t.getMessage());
		}
	}
	...
	private class RedirectHandler extends AbstractHandler {
		...
		/**
		 * Handles all requests by redirecting them (with a 301) to the https location
		...
		public void handle(String target, Request baseRequest, HttpServletRequest request, HttpServletResponse response)
			...
			String location = "https://" + host + ":" + targetPort + target;
