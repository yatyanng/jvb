...
public class JvbBundleConfig extends OSGiBundleConfig {
	 ...
	 * <tt>BundleActivator</tt> implementations) comprising Jitsi Videobridge. An
	 * element of the <tt>BUNDLES</tt> array is an array of <tt>String</tt>s and
	 * represents an OSGi start level.
	 */
	private static final String[][] BUNDLES = { { "org/jitsi/eventadmin/Activator" },
			...
			{ "org/jitsi/videobridge/xmpp/ClientConnectionImpl" }, { "org/jitsi/videobridge/octo/OctoRelayService" } };
	...
	public Map<String, String> getSystemPropertyDefaults() {
		...
		defaults.put(DeviceConfiguration.PROP_VIDEO_RTP_PACING_THRESHOLD, Integer.toString(Integer.MAX_VALUE));
		...
		defaults.put(StackProperties.CONSENT_FRESHNESS_ORIGINAL_WAIT_INTERVAL, "500");
		defaults.put(StackProperties.CONSENT_FRESHNESS_MAX_WAIT_INTERVAL, "500");
		// Retry max 5 times which will take up to 2500ms, that is before
		// the next "consent freshness" transaction starts
		defaults.put(StackProperties.CONSENT_FRESHNESS_MAX_RETRANSMISSIONS, "5");
		...
		defaults.put(PacketLoggingConfiguration.PACKET_LOGGING_ENABLED_PROPERTY_NAME, false_);
		...
		defaults.put(SimulcastController.ENABLE_VP8_PICID_REWRITING_PNAME, true_);
