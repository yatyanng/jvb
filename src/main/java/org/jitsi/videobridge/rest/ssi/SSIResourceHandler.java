...
import org.jitsi.videobridge.rest.PublicRESTBundleActivator;
...
public class SSIResourceHandler extends ResourceHandler {
	...
	/**
	 * The list of targets which will be processed by this {@code ResourceHandler};
	...
	public SSIResourceHandler(ConfigurationService cfg) {
		...
		String paths = ConfigUtils.getString(cfg,
				PublicRESTBundleActivator.JETTY_PROPERTY_PREFIX + JETTY_SSI_RESOURCE_HANDLER_PATHS,
				OLD_PREFIX + JETTY_SSI_RESOURCE_HANDLER_PATHS, null);
	...
	/**
	 * Processes the request to serve a file while checking it for ssi replacement
	 * tags.
	 *
	 * @param target      the target location
	 * @param baseRequest the base request
	 ...
	 * @throws ServletException
	 */
	private void handleSSIRequest(String target, Request baseRequest, HttpServletRequest request,
		...
		byte[] processedResult = processContentForServerSideIncludes(servletResponseWrapper.getContent());
	...
	/**
	 * Processes the current content and searches for ssi tags to replace them with
	 * the content. Currently, only include virtual is supported.
	 *
	...
	private byte[] processContentForServerSideIncludes(byte[] content) throws IOException {
		...
		while (scanner.hasNext()) {
			...
			if (startIx != -1) {
				int endIx = line.indexOf(SSI_CMD_END, startIx + SSI_CMD_START.length());
	...
	private boolean processSSICmd(String cmd, OutputStream out) throws IOException {
		...
		if (cmd.startsWith(SSI_CMD_INCLUDE) && cmd.contains("=")) {
			...
			if (SSI_PARAM_VIRTUAL.equals(parameterName)) {
				...
				// Alias check.
				String property = PublicRESTBundleActivator.JETTY_RESOURCE_HANDLER_ALIAS_PREFIX + "." + fileToInclude;
				String aliasValue = ConfigUtils.getString(cfg,
						PublicRESTBundleActivator.JETTY_PROPERTY_PREFIX + property, OLD_PREFIX + property, null);
				if (aliasValue != null)
