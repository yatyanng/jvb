...
public class VideobridgeStatistics extends Statistics {
	...
	/**
	 * The <tt>DateFormat</tt> to be utilized by <tt>VideobridgeStatistics</tt> in
	 * order to represent time and date as <tt>String</tt>.
	 */
	...
	/**
	 * The name of the stat that indicates the bridge has entered graceful shutdown
	 * mode. Its runtime type is {@code Boolean}.
	 */
	public static final String SHUTDOWN_IN_PROGRESS = "graceful_shutdown";

	/**
	 * The name of the piece of statistic which specifies the date and time at which
	 * the associated set of statistics was generated. Its runtime type is
	...
	/**
	 * The name of the total number of channels where the transport failed to be
	 * established.
	 */
	...
	/**
	 * The name of the total number of conferences where all channels failed due to
	 * no payload traffic.
	 */
	...
	/**
	 * The name of the stat indicating the total number of conference-seconds (i.e.
	 * the sum of the lengths is seconds).
	 */
	private static final String TOTAL_CONFERENCE_SECONDS = "total_conference_seconds";

	/**
	 * The name of the stat indicating the total number of participant-seconds that
	 * are loss-controlled (i.e. the sum of the lengths is seconds).
	 */
	...
	/**
	 * The name of the stat indicating the total number of messages sent over data
	 * channels.
	 */
	...
	/**
	 * The name of the stat indicating the total number of bytes received in Octo
	 * packets.
	 */
	...
	/**
	 * The indicator which determines whether {@link #generate()} is executing on
	 * this <tt>VideobridgeStatistics</tt>. If <tt>true</tt>, invocations of
	 * <tt>generate()</tt> will do nothing. Introduced in order to mitigate an issue
	 * in which a blocking in <tt>generate()</tt> will cause a multiple of threads
	 * to be initialized and blocked.
	 */
	...
	protected void generate0() {
		...
		int totalConferencesCreated = 0, totalConferencesCompleted = 0, totalFailedConferences = 0,
				totalPartiallyFailedConferences = 0, totalNoTransportChannels = 0, totalNoPayloadChannels = 0,
				totalChannels = 0;
		long totalConferenceSeconds = 0, totalLossControlledParticipantSeconds = 0,
				totalLossLimitedParticipantSeconds = 0, totalLossDegradedParticipantSeconds = 0;
		...
		OctoRelayService relayService = ServiceUtils.getService(bundleContext, OctoRelayService.class);
		...
		for (Videobridge videobridge : Videobridge.getVideobridges(bundleContext)) {
			...
			totalLossControlledParticipantSeconds += jvbStats.totalLossControlledParticipantMs.get() / 1000;
			totalLossLimitedParticipantSeconds += jvbStats.totalLossLimitedParticipantMs.get() / 1000;
			totalLossDegradedParticipantSeconds += jvbStats.totalLossDegradedParticipantMs.get() / 1000;
			...
			for (Conference conference : videobridge.getConferences()) {
				...
				int idx = conferenceEndpoints < conferenceSizes.length ? conferenceEndpoints
		...
		// Loss rates
		double lossRateDownload = (packetsReceived + packetsReceivedLost > 0)
				? ((double) packetsReceivedLost) / (packetsReceived + packetsReceivedLost)
				: 0d;
		double lossRateUpload = (fractionLostCount > 0) ? fractionLostSum / fractionLostCount : 0d;

		// JITTER_AGGREGATE
		double jitterAggregate = jitterCount > 0 ? jitterSumMs / jitterCount : 0;

		// RTT_AGGREGATE
		double rttAggregate = rttCount > 0 ? ((double) rttSumMs) / rttCount : 0;
		...
		try {
			...
			unlockedSetStat(RTP_LOSS, lossRateDownload + lossRateUpload);
			...
			unlockedSetStat(TOTAL_PARTIALLY_FAILED_CONFERENCES, totalPartiallyFailedConferences);
			unlockedSetStat(TOTAL_NO_PAYLOAD_CHANNELS, totalNoPayloadChannels);
			unlockedSetStat(TOTAL_NO_TRANSPORT_CHANNELS, totalNoTransportChannels);
			unlockedSetStat(TOTAL_CONFERENCES_CREATED, totalConferencesCreated);
			unlockedSetStat(TOTAL_CONFERENCES_COMPLETED, totalConferencesCompleted);
			...
			unlockedSetStat(TOTAL_LOSS_CONTROLLED_PARTICIPANT_SECONDS, totalLossControlledParticipantSeconds);
			unlockedSetStat(TOTAL_LOSS_LIMITED_PARTICIPANT_SECONDS, totalLossLimitedParticipantSeconds);
			unlockedSetStat(TOTAL_LOSS_DEGRADED_PARTICIPANT_SECONDS, totalLossDegradedParticipantSeconds);
			...
			unlockedSetStat(TOTAL_DATA_CHANNEL_MESSAGES_RECEIVED, totalDataChannelMessagesReceived);
			unlockedSetStat(TOTAL_DATA_CHANNEL_MESSAGES_SENT, totalDataChannelMessagesSent);
			unlockedSetStat(TOTAL_COLIBRI_WEB_SOCKET_MESSAGES_RECEIVED, totalColibriWebSocketMessagesReceived);
			unlockedSetStat(TOTAL_COLIBRI_WEB_SOCKET_MESSAGES_SENT, totalColibriWebSocketMessagesSent);
			...
			unlockedSetStat(TOTAL_PACKETS_RECEIVED_OCTO, totalPacketsReceivedOcto);
