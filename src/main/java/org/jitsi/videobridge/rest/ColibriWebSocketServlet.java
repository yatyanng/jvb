...
class ColibriWebSocketServlet extends WebSocketServlet {
	...
	public void configure(WebSocketServletFactory webSocketServletFactory) {
		...
		webSocketServletFactory.setCreator(new WebSocketCreator() {
			...
			public Object createWebSocket(ServletUpgradeRequest request, ServletUpgradeResponse response) {
				try {
					return ColibriWebSocketServlet.this.createWebSocket(request, response);
	 ...
	 */
	private ColibriWebSocket createWebSocket(ServletUpgradeRequest request, ServletUpgradeResponse response)
			throws IOException {
		...
		path = path.substring(ColibriWebSocketService.COLIBRI_WS_PATH.length(), path.length());
		...
		if (!serverId.equals(ids[0])) {
			logger.warn("Received request with a mismatching server ID " + "(expected '" + serverId + "', received '"
					+ ids[0] + "').");
		...
		if (abstractEndpoint == null || !(abstractEndpoint instanceof Endpoint)) {
			logger.warn("Received request for a nonexistent endpoint: " + ids[1] + "(conference " + conference.getID());
			response.sendError(403, authFailed);
			return null;
		}
