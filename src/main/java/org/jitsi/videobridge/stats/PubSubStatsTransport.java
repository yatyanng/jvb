...
public class PubSubStatsTransport extends StatsTransport implements PubSubResponseListener {
	/**
	 * The <tt>Logger</tt> used by the <tt>PubSubStatsTransport</tt> class and its
	 * instances to print debug information.
	 */
	private static final Logger logger = Logger.getLogger(PubSubStatsTransport.class);

	/**
	 * The ID of PubSub item which stores bridge statistics. As a value the JID of
	 * first currently registered <tt>ComponentImpl</tt> service will be used.
	 */
	...
	/**
	 * The <tt>ServiceListener</tt> which listens to the <tt>BundleContext</tt> in
	 * which this <tt>StatsTransport</tt> is started in order to track when
	 ...
	 * {@inheritDoc}
	 *
	 * Initializes this <tt>StatsTransport</tt> if it has not been initialized yet.
	 */
	@Override
	...
	/**
	 * Disposes of this instance by releasing the resources it has acquired by now
	 * and, effectively, preparing it for garbage collection.
	 */
	 ...
	 * publisher if this <tt>StatsTransport</tt> does not have an associated PubSub
	 * publisher yet; otherwise, does nothing.
	 */
	private void init() {
		if (publisher == null) {
	...
	/**
	 * Invokes either {@link #init()} or {@link #dispose()} depending on the state
	 * of this instance and on whether the method invocation is the result of the
	 * unregistering of a specific <tt>ComponentImpl</tt>.
	 *
	 * @param unregistering the <tt>ComponentImpl</tt> which is unregistering and
	 *                      thus has caused the method invocation if any; otherwise,
	 *                      <tt>null</tt>
	 */
	...
	public void onPublishResponse(Response type, IQ iq) {
		if (Response.FAIL.equals(type)) {
			...
			if (err != null && XMPPError.Type.CANCEL.equals(err.getType())
					&& XMPPError.Condition.item_not_found.equals(err.getCondition())) {
	...
	public void publishStatistics(Statistics stats) {
		...
		if (publisher != null) {
			try {
				...
				logger.error("Failed to publish to PubSub node: " + nodeName + " - it does not exist yet");
			} catch (Exception e) {
				logger.error("Failed to publish to PubSub node: " + nodeName, e);
				dispose();
			}
		}
	}
	...
	/**
	 * Notifies this instance that there was an OSGi service-related change in the
	 * <tt>BundleContext</tt> in which this <tt>StatsTransport</tt> is started.
	 * Initializes or disposes of this <tt>StatsTransport</tt> depending on whether
	 * there is a Jabber component (protocol implementation) registered in the
	 * <tt>BundleContext</tt> because this <tt>StatsTransport</tt> utilizes XMPP to
	...
	private void serviceChanged(ServiceEvent ev) {
		...
		if ((type == ServiceEvent.REGISTERED) || (type == ServiceEvent.UNREGISTERING)) {
			BundleContext bundleContext = getBundleContext();
			...
			if (bundleContext != null) {
				...
				if (service instanceof ComponentImpl) {
					initOrDispose((type == ServiceEvent.UNREGISTERING) ? (ComponentImpl) service : null);
				}
			}
		}
	}
