...
public class StatsManagerBundleActivator implements BundleActivator {
	/**
	 * The <tt>BundleContext</tt> in which a <tt>StatsManagerBundleActivator</tt>
	 * has been started.
	 */
	...
	/**
	 * The <tt>Logger</tt> used by the <tt>StatsManagerBundleActivator</tt> class
	 ...
	 * will receive the statistics about the Videobridge if PubSub transport is used
	 * to send statistics.
	 */
	private static final String PUBSUB_NODE_PNAME = "org.jitsi.videobridge.PUBSUB_NODE";

	/**
	 * The name of the property which specifies the name of the service that will
	 * receive the statistics about the Videobridge if PubSub transport is used to
	 * send statistics.
	 */
	...
	/**
	 * The name of the property which specifies the transport for sending statistics
	 * about the Videobridge.
	 */
	...
	/**
	 * The <tt>ServiceRegistration</tt> of a <tt>StatsManager</tt> instance within
	 ...
	 * newly-added {@code StatsTransport} in to repeatedly send {@code Statistics}
	 * at a specific {@code interval}.
	 *
	 * @param statsMgr  the {@code StatsManager} to add the new
	 *                  {@code StatsTransport} to
	 * @param cfg       the {@code ConfigurationService} to read property values
	 *                  from or {@code null} to read the property values from
	 *                  {@code System}
	 * @param interval  the interval/period in milliseconds at which the
	 *                  newly-initialized and added {@code StatsTransport} is to
	 *                  repeatedly send {@code Statistics}
	 * @param transport the identifier of the {@code StatsTransport} to initialize
	 *                  and add to {@code statsMgr}
	 */
	private void addTransport(StatsManager statsMgr, ConfigurationService cfg, int interval, String transport) {
		...
		if (t != null) {
			...
			interval = ConfigUtils.getInt(cfg, STATISTICS_INTERVAL_PNAME + "." + transport, interval);
	 ...
	 *                 or {@code null} to read the property values from
	 *                 {@code System}
	 * @param interval the interval/period in milliseconds at which the
	 *                 newly-initialized and added {@code StatsTransport}s to
	 *                 repeatedly send {@code Statistics}
	 */
	private void addTransports(StatsManager statsMgr, ConfigurationService cfg, int interval) {
		String transports = ConfigUtils.getString(cfg, STATISTICS_TRANSPORT_PNAME, DEFAULT_STAT_TRANSPORT);
	 ...
	 * Initializes and starts a new <tt>StatsManager</tt> instance and registers it
	 * as an OSGi service in the specified <tt>bundleContext</tt>.
	 *
	 ...
	 * as an OSGi service in the specified <tt>bundleContext</tt>.
	 *
	 * @param cfg the <tt>ConfigurationService</tt> in the <tt>BundleContext</tt> in
	 *            which the <tt>StatsManager</tt> OSGi is to start
	 */
	private void start(ConfigurationService cfg) throws Exception {
		StatsManager statsMgr = new StatsManager();
		int interval = ConfigUtils.getInt(cfg, STATISTICS_INTERVAL_PNAME, DEFAULT_STAT_INTERVAL);
	 ...
	 * <tt>BundleContext</tt> if such an instance has been registered and started.
	 *
	 * @param bundleContext the <tt>BundleContext</tt> in which the
	...
	public void stop(BundleContext bundleContext) throws Exception {
		...
		try {
			statsMgr = bundleContext.getService(serviceRegistration.getReference());
		} finally {
