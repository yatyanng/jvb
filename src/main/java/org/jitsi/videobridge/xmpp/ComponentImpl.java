...
public class ComponentImpl extends ComponentBase implements BundleActivator {
	/**
	 * The {@link Logger} used by the {@link ComponentImpl} class and its instances
	 * for logging output.
	 */
	private static final org.jitsi.util.Logger logger = org.jitsi.util.Logger.getLogger(ComponentImpl.class);
	...
	/**
	 * Gets the <tt>ComponentImpl</tt> instances registered as OSGi services in a
	 * specific <tt>BundleContext</tt>.
	 *
	 * @param bundleContext the <tt>BundleContext</tt> from which the registered
	 *                      <tt>ComponentImpl</tt> instances registered as OSGi
	 *                      services are to be returned
	 * @return the <tt>ComponentImpl</tt> instances registered as OSGi services in
	 ...
	 *                  connect.
	 * @param domain    the name of main XMPP domain on which this component will be
	 *                  served.
	 * @param subDomain the name of subdomain on which this component will be
	 *                  available.
	 * @param secret    the password used by the component to authenticate with XMPP
	 *                  server.
	 */
	 ...
	 * {@inheritDoc}
	 *
	 * Gets the namespaces of features that this <tt>Component</tt> offers/supports
	 * i.e. {@link ColibriConferenceIQ#NAMESPACE}.
	 */
	@Override
	 ...
	 * {@inheritDoc}
	 *
	 * Gets the type of the Service Discovery Identity of this <tt>Component</tt>
	 * i.e. &quot;conference&quot;.
	 */
	@Override
	 ...
	 * <tt>org.xmpp.packet.IQ</tt> to an <tt>org.jivesoftware.smack.packet.IQ</tt>
	 * stanza, handles the Smack stanza via a call to {@link XmppCommon#handleIQ},
	 * converts the result Smack stanza to an <tt>org.xmpp.packet.iQ</tt> which is
		...
		try {
			...
			if (smackIQ == null) {
				if (iq.isRequest()) {
					...
					error.setError(new PacketError(PacketError.Condition.bad_request, PacketError.Type.modify,
			...
			org.jivesoftware.smack.packet.IQ resultSmackIQ = common.handleIQ(smackIQ);
			IQ resultIQ;
	 ...
	 *
	 * @param iq the <tt>org.xmpp.packet.IQ</tt> stanza of type <tt>get</tt> which
	...
	/**
	 * Called as part of the execution of {@link AbstractComponent#shutdown()} to
	 * enable this <tt>Component</tt> to finish cleaning resources up after it gets
	 * completely shutdown.
	 *
	 ...
	 * enable this <tt>Component</tt> to finish initializing resources after it gets
	 * completely started.
	 *
	 ...
	 * @throws Exception if an error occurs during the conversion of the specified
	 *                   <tt>iq</tt> to an <tt>org.xmpp.packet.IQ</tt> instance or
	 *                   while sending the specified <tt>iq</tt>
	 */
	public void send(org.jivesoftware.smack.packet.IQ iq) throws Exception {
		try {
			/*
			 * The javadoc on ComponentManager.sendPacket(Component,Packet) which is invoked
			 * by AbstractComponent.send(Packet) says that the value of the from property of
			 * the Packet must not be null; otherwise, an IllegalArgumentException will be
			 * thrown.
			 */
			Jid from = iq.getFrom();
	 ...
	 *                      component implementation and the Jitsi Videobridge it
	 *                      provides are to be started
	 * @throws Exception if anything irreversible goes wrong while starting this
	 *                   Jabber component implementation and the Jitsi Videobridge
	 *                   it provides in the specified <tt>bundleContext</tt>
	 */
	@Override
