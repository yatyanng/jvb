...
final class JSONSerializer {
	 ...
	 * <tt>candidateList</tt> property of <tt>IceUdpTransportPacketExtension</tt>.
	 */
	...
	/**
	 * The name of the JSON pair which specifies the value of the <tt>endpoints</tt>
	 * property of <tt>ColibriConferenceIQ</tt>.
	 */
	static final String ENDPOINTS = ColibriConferenceIQ.Endpoint.ELEMENT_NAME + "s";

	/**
	 * The name of the JSON pair which specifies the value of the <tt>channels</tt>
	 * property of <tt>ColibriConferenceIQ.Content</tt>.
	 */
	static final String CHANNELS = ColibriConferenceIQ.Channel.ELEMENT_NAME + "s";

	/**
	 * The name of the JSON pair which specifies the value of the <tt>contents</tt>
	 * property of <tt>ColibriConferenceIQ</tt>.
	 */
	...
	/**
	 * The name of the JSON pair which specifies the value of the <tt>rtcp-fb</tt>
	 * property of <tt>ColibriConferenceIQ.Channel</tt>.
	 */
	static final String RTCP_FBS = RtcpFbPacketExtension.ELEMENT_NAME + "s";

	/**
	 * The name of the JSON pair which specifies the value of the
	 * <tt>sctpConnections</tt> property of <tt>ColibriConferenceIQ.Content</tt>.
	 */
	...
	/**
	 * The name of the JSON pair which specifies the value of the <tt>sources</tt>
	 * property of <tt>ColibriConferenceIQ.Channel</tt>.
	 */
	static final String SOURCES = SourcePacketExtension.ELEMENT_NAME + "s";

	/**
	 * The name of the JSON pair which specifies the value of the <tt>ssrcs</tt>
	 * property of <tt>ColibriConferenceIQ.Channel</tt>.
	 */
	 ...
	 */
	static final String WEBSOCKET_LIST = WebSocketPacketExtension.ELEMENT_NAME + "s";

	/**
	 * The name of the JSON pair which specifies the value of the <tt>namespace</tt>
	 * property of <tt>IceUdpTransportPacketExtension</tt>.
	 */
	static final String XMLNS = "xmlns";

	/**
	 * Serializes the attribute values of an <tt>AbstractPacketExtension</tt> into
	 * values of a <tt>JSONObject</tt>.
	 *
	 * @param abstractPacketExtension the <tt>AbstractPacketExtension</tt> whose
	 *                                attribute values are to be serialized into
	 *                                values of <tt>jsonObject</tt>
	 * @param jsonObject              the <tt>JSONObject</tt> into which the
	 *                                attribute values of
	 *                                <tt>abstractPacketExtension</tt> are to be
	 *                                serialized
	 */
	public static void serializeAbstractPacketExtensionAttributes(AbstractPacketExtension abstractPacketExtension,
		...
		for (String name : abstractPacketExtension.getAttributeNames()) {
			...
			/*
			 * The JSON.simple library that is in use at the time of this writing will fail
			 * to encode Enum values as JSON strings so convert the Enum value to a Java
			 * String.
			 */
	...
	public static JSONObject serializeChannel(ColibriConferenceIQ.Channel channel) {
		...
		if (channel == null) {
			...
			if (rtpLevelRelayType != null) {
				 ...
				 * String.
				 */
				jsonObject.put(ColibriConferenceIQ.Channel.RTP_LEVEL_RELAY_TYPE_ATTR_NAME,
	...
	public static JSONObject serializeEndpoint(ColibriConferenceIQ.Endpoint endpoint) {
		...
		if (endpoint == null) {
			...
			// id
			if (id != null) {
				jsonObject.put(ColibriConferenceIQ.Endpoint.ID_ATTR_NAME, id);
			}
			// statsId
			if (statsId != null) {
				jsonObject.put(ColibriConferenceIQ.Endpoint.STATS_ID_ATTR_NAME, statsId);
			}
			// displayName
			if (displayName != null) {
				jsonObject.put(ColibriConferenceIQ.Endpoint.DISPLAYNAME_ATTR_NAME, displayName);
			}
		}
		return jsonObject;
	}
	...
	public static JSONObject serializeChannelCommon(ColibriConferenceIQ.ChannelCommon channelCommon) {
		...
		if (channelCommon == null) {
			...
			// id
			if (id != null) {
				jsonObject.put(ChannelCommon.ID_ATTR_NAME, id);
			}
			// channelBundleId
	...
	public static JSONObject serializeParameters(Collection<ParameterPacketExtension> parameters) {
		 ...
		 * payload-type does not appear to matter so a natural representation of a
		 * parameter set is a JSONObject rather than a JSONArray.
		 */
	...
	public static JSONArray serializeRtcpFbs(@NotNull Collection<RtcpFbPacketExtension> rtcpFbs) {
		...
		/*
		 * A rtcp-fb is an JSONObject with type / subtype data. "rtcp-fbs": [ { "type":
		 * "ccm", "subtype": "fir" }, { "type": "nack" }, { "type": "goog-remb" } ]
		 */
	...
	public static JSONObject serializePayloadType(PayloadTypePacketExtension payloadType) {
		...
		if (payloadType == null) {
			...
			if ((rtcpFeedbackTypeList != null) && !rtcpFeedbackTypeList.isEmpty()) {
				payloadTypeJSONObject.put(RTCP_FBS, serializeRtcpFbs(rtcpFeedbackTypeList));
			}
		}
	...
	public static JSONObject serializeTransport(IceUdpTransportPacketExtension transport) {
		...
		if (transport == null) {
			...
			List<WebSocketPacketExtension> webSocketList = transport
					.getChildExtensionsOfType(WebSocketPacketExtension.class);
			...
			if ((webSocketList != null) && (!webSocketList.isEmpty())) {
				jsonObject.put(WEBSOCKET_LIST, serializeWebSockets(webSocketList));
			}
			// rtcpMux
			if (rtcpMux) {
