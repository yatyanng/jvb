...
public class AudioChannel extends RtpChannel {
	...
	/**
	 * A boolean that indicates whether or not we've fetched the {@link LipSyncHack}
	 * from the {@link VideoChannel}.
	 */
	private boolean fetchedLipSyncHack = false;

	/**
	 * Initializes a new <tt>AudioChannel</tt> instance which is to have a specific
	 ...
	 */
	public AudioChannel(Content content, String id, String channelBundleId, String transportNamespace,
			Boolean initiator) {
	 ...
	 * {@inheritDoc}
	 * </p>
	 * Registers this channel's {@link AudioChannelAudioLevelListener} with the
	 * media stream.
	 */
	@Override
	...
	boolean rtpTranslatorWillWrite(boolean data, RawPacket pkt, RtpChannel source) {
		...
		if (!fetchedLipSyncHack) {
			...
			if (channels != null && !channels.isEmpty()) {
				associatedLipSyncHack = ((VideoChannel) channels.get(0)).getLipSyncHack();
			}
		}

		if (associatedLipSyncHack != null) {
			associatedLipSyncHack.onRTPTranslatorWillWriteAudio(pkt, source);
		}
