...
class EndpointMessageTransport extends AbstractEndpointMessageTransport implements WebRtcDataStream.DataCallback {
	...
	/**
	 * Whether the last active transport channel (i.e. the last to receive a message
	 * from the remote endpoint) was the web socket (if {@code true}), or the WebRTC
	 * data channel (if {@code false}).
	 */
	...
	/**
	 * The listener which will be added to {@link SctpConnection}s associated with
	 * this endpoint, which will be notified when {@link SctpConnection}s become
	 * ready.
	 *
	 * Note that we just want to make sure that the initial messages (e.g. a
	 * dominant speaker notification) are sent. Because of this we only add
	 * listeners to {@link SctpConnection}s which are not yet ready, and we remove
	 * the listener after an {@link SctpConnection} becomes ready.
	 */
		...
		public void onSctpConnectionReady(SctpConnection source) {
			if (source != null) {
				if (source.equals(getSctpConnection())) {
					try {
						WebRtcDataStream dataStream = source.getDefaultDataStream();
	 ...
	 * 
	 * @param dst          the {@link WebRtcDataStream} through which to send the
	 *                     message.
	 * @param message      the message to send.
	 * @param errorMessage an error message to be logged in case of failure.
	 */
	private void sendMessage(WebRtcDataStream dst, String message, String errorMessage) {
		try {
			...
			endpoint.getConference().getVideobridge().getStatistics().totalDataChannelMessagesSent.incrementAndGet();
		} catch (IOException ioe) {
			logger.error("Failed to send a message over a WebRTC data channel" + " (endpoint=" + endpoint.getID()
					+ "): " + errorMessage, ioe);
		}
	}
	 ...
	 * 
	 * @param dst          the {@link ColibriWebSocket} through which to send the
	 *                     message.
	 * @param message      the message to send.
	 * @param errorMessage an error message to be logged in case of failure.
	 */
	private void sendMessage(ColibriWebSocket dst, String message, String errorMessage) {
		...
		endpoint.getConference().getVideobridge().getStatistics().totalColibriWebSocketMessagesSent.incrementAndGet();
	}
	...
	protected void onPinnedEndpointsChangedEvent(Object src, JSONObject jsonObject) {
		...
		if (!(o instanceof JSONArray)) {
			logger.warn("Received invalid or unexpected JSON (" + endpoint.getLoggingId() + "):" + jsonObject);
			return;
		}
		...
		if (logger.isDebugEnabled()) {
			logger.debug(Logger.Category.STATISTICS,
					"pinned," + endpoint.getLoggingId() + " pinned=" + newPinnedEndpoints);
		}
	...
	public void onStringData(WebRtcDataStream src, String msg) {
		...
		endpoint.getConference().getVideobridge().getStatistics().totalDataChannelMessagesReceived.incrementAndGet();
	 ...
	 *         </p>
	 *         The "active" channel is determined based on what channels are
	 *         available, and which one was the last to receive data. That is, if
	 *         only one channel is available, it will be returned. If two channels
	 *         are available, the last one to have received data will be returned.
	 *         Otherwise, {@code null} will be returned.
	 */
	private Object getActiveTransportChannel() throws IOException {
		...
		if (dst == null) {
			if (sctpConnection != null && sctpConnection.isReady()) {
				...
				if (dst == null) {
					logger.warn("SCTP ready, but WebRtc data channel with " + endpointId + " not opened yet.");
				}
			} else {
				logger.warn("SCTP connection with " + endpointId + " not ready yet.");
			}
		}
	 ...
	 * {@link ColibriWebSocket} instance associated with its {@link Endpoint} has
	 * connected.
	 * 
	...
	public void onWebSocketClose(ColibriWebSocket ws, int statusCode, String reason) {
		synchronized (webSocketSyncRoot) {
			if (ws != null && ws.equals(webSocket)) {
				...
				if (logger.isDebugEnabled()) {
					logger.debug(
							"Web socket closed for endpoint " + endpoint.getID() + ": " + statusCode + " " + reason);
				}
			}
		}
	 ...
	 * received from a specific {@link ColibriWebSocket} instance associated with
	 * its {@link Endpoint}.
	 * 
	...
	public void onWebSocketText(ColibriWebSocket ws, String message) {
		if (ws == null || !ws.equals(webSocket)) {
			logger.warn("Received text from an unknown web socket " + "(endpoint=" + endpoint.getID() + ").");
			return;
		}

		endpoint.getConference().getVideobridge().getStatistics().totalColibriWebSocketMessagesReceived
				.incrementAndGet();
