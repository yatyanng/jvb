...
public class LipSyncHack implements TransformEngine {
	/**
	 * A byte array holding a black VP8 key frame. The byte array contains the full
	 * RTP packet and not just the VP8 payload (160 bytes).
	 */
	public static final byte[] KEY_FRAME_BUFFER = new byte[] {

			// RTP
			(byte) 0x90, (byte) 0xe4, (byte) 0x38, (byte) 0x25, (byte) 0x53, (byte) 0x50, (byte) 0x6c, (byte) 0x8b,
			(byte) 0x8e, (byte) 0x71, (byte) 0xf1, (byte) 0x72, (byte) 0xbe, (byte) 0xde, (byte) 0x00, (byte) 0x01,
			(byte) 0x32, (byte) 0x62, (byte) 0x45, (byte) 0xaa,

			// VP8 PLD
			(byte) 0x90, (byte) 0xe0, (byte) 0x80, (byte) 0x01, (byte) 0x00, (byte) 0x20, (byte) 0x10, (byte) 0x0f,
			(byte) 0x00, (byte) 0x9d, (byte) 0x01, (byte) 0x2a, (byte) 0x40, (byte) 0x01, (byte) 0xb4, (byte) 0x00,
			(byte) 0x07, (byte) 0x07, (byte) 0x09, (byte) 0x03, (byte) 0x0b, (byte) 0x0b, (byte) 0x11, (byte) 0x33,
			(byte) 0x09, (byte) 0x10, (byte) 0x4b, (byte) 0x00, (byte) 0x00, (byte) 0x0c, (byte) 0x2c, (byte) 0x09,
			(byte) 0xee, (byte) 0x0d, (byte) 0x02, (byte) 0xc9, (byte) 0x3e, (byte) 0xd7, (byte) 0xb7, (byte) 0x36,
			(byte) 0x4e, (byte) 0x70, (byte) 0xf6, (byte) 0x4e, (byte) 0x70, (byte) 0xf6, (byte) 0x4e, (byte) 0x70,
			(byte) 0xf6, (byte) 0x4e, (byte) 0x70, (byte) 0xf6, (byte) 0x4e, (byte) 0x70, (byte) 0xf6, (byte) 0x4e,
			(byte) 0x70, (byte) 0xf6, (byte) 0x4e, (byte) 0x70, (byte) 0xf6, (byte) 0x4e, (byte) 0x70, (byte) 0xf6,
			(byte) 0x4e, (byte) 0x70, (byte) 0xf6, (byte) 0x4e, (byte) 0x70, (byte) 0xf6, (byte) 0x4e, (byte) 0x70,
			(byte) 0xf6, (byte) 0x4e, (byte) 0x70, (byte) 0xf6, (byte) 0x4e, (byte) 0x70, (byte) 0xf6, (byte) 0x4e,
			(byte) 0x70, (byte) 0xf6, (byte) 0x4e, (byte) 0x70, (byte) 0xf6, (byte) 0x4e, (byte) 0x70, (byte) 0xf6,
			(byte) 0x4e, (byte) 0x70, (byte) 0xf6, (byte) 0x4e, (byte) 0x70, (byte) 0xf6, (byte) 0x4e, (byte) 0x70,
			...
			(byte) 0x4e, (byte) 0x70, (byte) 0xf6, (byte) 0x4e, (byte) 0x70, (byte) 0xf6, (byte) 0x4e, (byte) 0x70,
			(byte) 0xf6, (byte) 0x4e, (byte) 0x70, (byte) 0xf6, (byte) 0x4e, (byte) 0x70, (byte) 0xf6, (byte) 0x4e,
			(byte) 0x70, (byte) 0xf6, (byte) 0x4e, (byte) 0x70, (byte) 0xf6, (byte) 0x4e, (byte) 0x5c, (byte) 0x00,
	 ...
	 * injection period.
	 */
	public final static String KEY_FRAMES_PER_INJECTION_PNAME = "org.jitsi.videobridge.LipSyncHack.KEY_FRAMES_PER_INJECTION_PNAME";

	/**
	 * Configuration property for the black key frames send period (in ms).
	 */
	public final static String PERIOD_MS_PNAME = "org.jitsi.videobridge.LipSyncHack.PERIOD_MS_PNAME";
	 ...
	 */
	private static final long TS_INCREMENT_PER_FRAME = 90000 /* Hz */ / 30 /* fps */;

	/**
	 * The <tt>Logger</tt> used by the <tt>LipSyncHack</tt> class and its instances
	 * to print debug information.
	 */
	private static final Logger logger = Logger.getLogger(LipSyncHack.class);

	/**
	 * The {@link Random} that will be used to generate the random sequence number
	 * and RTP timestamp offsets.
	 */
	...
	/**
	 * The {@link PacketTransformer} that rewrites RTP or prepends RTP streams with
	 * black key frames.
	 */
	...
	/**
	 * Listens for RTCP report blocks and stops the injection if we receive an RTCP
	 * report for an SSRC that we're hacking.
	 */
	 ...
	 * forwarded to the endpoint associated to this instance. Accessed by the audio
	 * translator thread only (no sync needed).
	 */
	 ...
	 * forwarded to the endpoint associated to this instance. Accessed by the video
	 * translator thread only (no sync needed).
	 */
	 ...
	 * written. The purpose of this is to trigger the hack for the video stream that
	 * is associated to the audio SSRC that is about to be written.
	 *
	...
	void onRTPTranslatorWillWriteAudio(RawPacket pkt, RtpChannel source) {
		...
		MediaStream stream;
		if (dest == null || (stream = dest.getStream()) == null || !stream.isStarted()) {
			// It seems like we're not ready yet to trigger the hack.
			return;
		}

		MediaStreamTrackDesc[] sourceTracks = source.getEndpoint().getMediaStreamTracks(MediaType.VIDEO);
		...
		synchronized (filterSyncRoot) {
			if (acceptedVideoSSRCs.contains(receiveVideoSSRC) || injections.containsKey(receiveVideoSSRC)) {
			...
			if (rtcpReportListener == null) {
				...
				stream.getMediaStreamStats().getRTCPReports().addRTCPReportListener(rtcpReportListener);
			}
		}
	}
	 ...
	 * @param seqNumOff the sequence number offset of the black key frame packets.
	...
	private RawPacket[] make(int ssrc, int seqNumOff, long tsOff, int sz) {
		if (vp8pt == -1) {
			vp8pt = dest.getStream().getDynamicRTPPayloadType(Constants.VP8) & 0xFF;
		}
	 ...
	 * written. The purpose of this is to stop the hack for the SSRC that is about
	 * to be written.
	 *
	...
	void onRTPTranslatorWillWriteVideo(RawPacket pkt, RtpChannel source) {
		...
		synchronized (filterSyncRoot) {
			...
			MediaStreamTrackDesc[] sourceTracks = source.getStream().getMediaStreamTrackReceiver()
			...
			long tsDelta = (injectState.maxTs + 10 * TS_INCREMENT_PER_FRAME - ts) & 0xFFFFFFFFL;

			if (logger.isDebugEnabled()) {
				logger.debug("new_translation" + ",stream=" + dest.getStream().hashCode() + " ssrc=" + acceptedVideoSSRC
						+ ",ts_delta=" + tsDelta + ",seq_num_delta=" + seqNumDelta);
			}

			// Setup packet transformation.
			transformations.put(acceptedVideoSSRC,
					new SeqNumPacketTranslation(seqNumDelta).andThen(new TimestampPacketTranslation(tsDelta)));
		}
	}
	...
	private class RTPTransformer implements PacketTransformer {
		...
		public void close() {
			...
			if (rtcpReportListener != null) {
				dest.getStream().getMediaStreamStats().getRTCPReports().removeRTCPReportListener(rtcpReportListener);
			}
		}
		...
		public RawPacket[] transform(RawPacket[] pkts) {
			...
			for (int i = 0; i < pkts.length; i++) {
				if (pkts[i] == null || !RTPPacketPredicate.INSTANCE.test(pkts[i])) {
					continue;
				}

				Long ssrc = pkts[i].getSSRCAsLong();
				AbstractFunction<RawPacket, RawPacket> transformation = transformations.get(ssrc);
				...
				if (transformation == null) {
					...
					long tsOff = (pkts[i].getTimestamp() - 10 * TS_INCREMENT_PER_FRAME) & 0xFFFFFFFFL;
					RawPacket[] extras = make(ssrc.intValue(), seqNumOff, tsOff, 10);

					cumulExtras = ArrayUtils.concat(cumulExtras, extras);

					if (logger.isDebugEnabled()) {
						logger.debug("new_translation" + ",stream=" + dest.getStream().hashCode() + " ssrc=" + ssrc
								+ ",pt=" + vp8pt + ",ts_delta=0,seq_num_delta=0");
					}
	...
	private class Injection implements RecurringRunnable {
		...
		/**
		 * The maximum sequence number (mod 2^16) that has been sent out by this
		 * injector instance.
		 */
		int maxSeqNum = -1;

		/**
		 * The maximum RTP timstamp (mod 2^32) that has been sent out by this injector
		 * instance.
		 */
		long maxTs = -1;

		/**
		 * A boolean indicating whether or not this instance is still injecting packets.
		 */
		...
		public long getTimeUntilNextRun() {
			...
			long timeSinceLastProcess = Math.max(System.currentTimeMillis() - lastRunMs, 0);
		...
		public void run() {
			...
			synchronized (this) {
				...
				if (maxSeqNum == -1) {
					...
					maxTs = (maxTs + KEY_FRAMES_PER_PERIOD * TS_INCREMENT_PER_FRAME) & 0xFFFFFFFFL;
				}
			}

			long tsOff = (maxTs - (KEY_FRAMES_PER_PERIOD - 1) * TS_INCREMENT_PER_FRAME) & 0xFFFFFFFFL;

			int seqNumOff = (maxSeqNum - KEY_FRAMES_PER_PERIOD - 1) & 0xFFFF;
			RawPacket[] kfs = make((int) receiveVideoSSRC, seqNumOff, tsOff, KEY_FRAMES_PER_PERIOD);
			...
			if (logger.isDebugEnabled()) {
				logger.debug("new_injection" + ",stream=" + stream.hashCode() + " ssrc=" + receiveVideoSSRC + ",max_ts="
						+ kfs[kfs.length - 1].getTimestamp() + ",max_seq_num="
						+ kfs[kfs.length - 1].getSequenceNumber());
			}
		}

		/**
		 * After stop is called, the injection logic is stopped and maxSeqNum and maxTs
	...
	class RTCPReportListener extends RTCPReportAdapter {
		...
		public void rtcpReportReceived(RTCPReport report) {
			for (RTCPFeedback feedback : report.getFeedbackReports()) {
				...
				if (injection != null && !transformations.containsKey(ssrc)) {
