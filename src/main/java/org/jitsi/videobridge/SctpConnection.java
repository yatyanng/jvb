...
public class SctpConnection extends Channel implements SctpDataCallback, SctpSocket.NotificationListener {
	...
	/**
	 * The {@link Logger} used by the {@link SctpConnection} class to print debug
	 ...
	 */
	private static final ExecutorService threadPool = ExecutorUtils.newCachedThreadPool(true,
	...
	/**
	 * Indicates whether the STCP association is ready and has not been ended by a
	 ...
	 * fired by this instance in order to prevent possible listeners from blocking
	 * this <tt>SctpConnection</tt> in general and {@link #sctpSocket} in particular
	 * for too long. The timeout of <tt>15</tt> is chosen to be in accord with the
	 * time it takes to expire a <tt>Channel</tt>.
	 */
	private final AsyncExecutor<Runnable> eventDispatcher = new AsyncExecutor<>(15, TimeUnit.MILLISECONDS);

	/**
	 * List of <tt>WebRtcDataStreamListener</tt>s that will be notified whenever new
	 * WebRTC data channel is opened.
	 */
	...
	/**
	 * The {@link PacketQueue} instance in which we place packets coming from the
	 * SCTP stack which are to be sent via {@link #transformer}.
	 */
	private final RawPacketQueue packetQueue;

	/**
	 * The {@link DtlsPacketTransformer} instance which we use to transport SCTP
	 * packets.
	 */
	private DtlsPacketTransformer transformer = null;

	/**
	 * The instance which we use to handle packets read from {@link #packetQueue}.
	 */
	 ...
	 *                        channel-bundle).
	 */
	public SctpConnection(String id, Content content, AbstractEndpoint endpoint, int remoteSctpPort,
			String channelBundleId, Boolean initiator) {
		super(content, id, channelBundleId, IceUdpTransportPacketExtension.NAMESPACE, initiator);

		logger = Logger.getLogger(classLogger, content.getConference().getLogger());
		setEndpoint(endpoint);
		packetQueue = new RawPacketQueue(false, getClass().getSimpleName() + "-" + endpoint.getID(), handler);
	...
	protected TransportManager createTransportManager(String xmlNamespace) throws IOException {
		if (IceUdpTransportPacketExtension.NAMESPACE.equals(xmlNamespace)) {
			...
			return new IceUdpTransportManager(content.getConference(), isInitiator(), 1 /* numComponents */,
					content.getName());
...
//            return new RawUdpTransportManager(this);
	 ...
	 *         <tt>null</tt> if there are no <tt>WebRtcDataStreamListener</tt>s
...
//                    def = null;
	...
	/**
	 * Submits {@link #notifyChannelOpenedInEventDispatcher(WebRtcDataStream)} to
	...
	private void notifyChannelOpened(final WebRtcDataStream dataChannel) {
		if (!isExpired()) {
			eventDispatcher.execute(() -> notifyChannelOpenedInEventDispatcher(dataChannel));
		}
	}
	...
	private void notifySctpConnectionReady() {
		if (!isExpired()) {
			eventDispatcher.execute(this::notifySctpConnectionReadyInEventDispatcher);
		}
	}

	/**
	 * Notifies the <tt>WebRtcDataStreamListener</tt>s added to this instance that
	 * this <tt>SctpConnection</tt> is ready i.e. it is connected to the remote peer
	 * and operational.
	 */
	private void notifySctpConnectionReadyInEventDispatcher() {
		/*
		 * When executing asynchronously in eventDispatcher, it is technically possible
		 * that this SctpConnection may have expired by now.
		 */
	...
	private void onCtrlPacketNotSynchronized(byte[] data, int sid) throws IOException {
		...
		if (messageType == MSG_CHANNEL_ACK) {
			if (logger.isDebugEnabled()) {
				logger.debug(Logger.Category.STATISTICS, "sctp_ack_received," + getLoggingId() + " sid=" + sid);
			}
			// Open channel ACK
			...
			if (channel != null) {
				...
				if (!channel.isAcknowledged()) {
					...
					logger.log(Level.WARNING, Logger.Category.STATISTICS,
							"sctp_redundant_ack_received," + getLoggingId() + " sid=" + sid);
				}
			} else {
				logger.error(Logger.Category.STATISTICS, "sctp_no_channel_for_sid," + getLoggingId() + " sid=" + sid);
			}
			...
			if (logger.isDebugEnabled()) {
				logger.debug(Logger.Category.STATISTICS,
						"dc_open_request," + getLoggingId() + " sid=" + sid + ",type=" + channelType + ",prio="
								+ priority + ",reliab=" + reliability + ",label=" + label + ",proto=" + protocol);
			}

			WebRtcDataStream.DataCallback oldCallback = null;
			if (channels.containsKey(sid)) {
				logger.log(Level.WARNING, Logger.Category.STATISTICS,
						"sctp_channel_exists," + getLoggingId() + " sid=" + sid);
			...
			if (logger.isDebugEnabled()) {
				...
				if (SctpNotification.SCTP_SENDER_DRY_EVENT != notification.sn_type) {
					logger.info(Logger.Category.STATISTICS,
							"sctp_notification," + getLoggingId() + " notification=" + notification);
				}
			}
	 ...
	 * 
	 * @param type   channel type as defined in control protocol description. Use 0
	 *               for "reliable".
	 * @param prio   channel priority. The higher the number, the lower the
	 *               priority.
	 * @param reliab Reliability Parameter<br/>
	 *
	 *               This field is ignored if a reliable channel is used. If a
	 *               partial reliable channel with limited number of retransmissions
	 *               is used, this field specifies the number of retransmissions. If
	 *               a partial reliable channel with limited lifetime is used, this
	 *               field specifies the maximum lifetime in milliseconds. The
	 *               following table summarizes this:<br/>
	 *               </br>
	 * 
	 *               +------------------------------------------------+------------------+
	 ...
	 *               |
	 *               +------------------------------------------------+------------------+
	 * @param sid    SCTP stream id that will be used by new channel (it must not be
	 *               already used).
	...
	/**
	 * Opens new WebRTC data channel using specified parameters. This should only be
	 * called while holding a lock on {@link #syncRoot}, as it does not obtain any
	 * locks on its own. See {@link #openChannel(int, int, long, int, String)} for a
	 * more detailed description.
	 */
	private WebRtcDataStream openChannelNotSynchronized(int type, int prio, long reliab, int sid, String label)
			throws IOException {
		if (channels.containsKey(sid))
	 ...
	 *
	 * @param listener the <tt>WebRtcDataStreamListener</tt> to be removed from the
	 *                 listeners list.
	 */
	...
	private void runOnDtlsTransport(StreamConnector connector) throws IOException {
		...
		if (LOG_SCTP_PACKETS) {
			System.setProperty(ConfigurationService.PNAME_SC_HOME_DIR_LOCATION, System.getProperty("java.io.tmpdir"));
		...
		sctpSocket.setLink(new NetworkLink() {
			...
			public void onConnOut(SctpSocket s, byte[] packet) throws IOException {
				if (LOG_SCTP_PACKETS) {
					LibJitsi.getPacketLoggingService().logPacket(PacketLoggingService.ProtocolName.ICE4J,
							new byte[] { 0, 0, 0, (byte) debugId }, 5000, new byte[] { 0, 0, 0, (byte) (debugId + 1) },
							remoteSctpPort, PacketLoggingService.TransportName.UDP, true, packet);
				}
		...
		threadPool.execute(new Runnable() {
			...
			public void run() {
				...
				try {
					...
					while (sctpSocket != null) {
						if (sctpSocket.accept()) {
							...
							logger.info("SCTP socket accepted for " + "endpoint " + getEndpoint().getID());
							break;
						}
						Thread.sleep(100);
						sctpSocket = SctpConnection.this.sctpSocket;
				...
				if (sctpSocket == null && logger.isInfoEnabled()) {
					logger.info("SctpConnection " + getID() + " closed" + " before SctpSocket accept()-ed.");
				}
			}
		});
		...
		try {
			do {
				...
				if (LOG_SCTP_PACKETS) {
					...
					byte[] srcAddr = new byte[] { 0, 0, 0, (byte) (debugId + 1) };
					...
					for (RawPacket s : send) {
						...
						pktLogging.logPacket(PacketLoggingService.ProtocolName.ICE4J, srcAddr, remoteSctpPort, dstAddr,
								5000, PacketLoggingService.TransportName.UDP, false, s.getBuffer(), s.getOffset(),
								s.getLength());
					}
				}
				...
				for (RawPacket s : send) {
					if (s != null) {
						sctpSocket.onConnIn(s.getBuffer(), s.getOffset(), s.getLength());
					}
				}
			} while (true);
	...
	/**
	 * A {@link org.ice4j.util.PacketQueue.PacketHandler} which sends packets over
	 * DTLS.
	 */
