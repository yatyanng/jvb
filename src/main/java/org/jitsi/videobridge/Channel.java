...
public abstract class Channel extends PropertyChangeNotifier implements Expireable {
	...
	/**
	 * The name of the <tt>Channel</tt> property <tt>endpoint</tt> which points to
	 ...
	 * information. Note that {@link Channel} instances should use {@link #logger}
	 * instead.
	 */
	private static final Logger classLogger = Logger.getLogger(Channel.class);

	/**
	 * @return a string which identifies a specific {@link Channel} for the purposes
	...
	public static String getLoggingId(Channel channel) {
		...
		return Content.getLoggingId(content) + ",ch_id=" + id + " (" + channel.hashCode() + ")" + ",endp_id="
	 ...
	 * <tt>Channel</tt>s listed in {@link #content} while this instance is listed
	 * there as well).
	 */
	 ...
	 * <tt>Channel</tt>. Currently this means when for the last time there were any
	 * RTP packets received for this channel or ICE "consent freshness check" has
	 * succeeded. In the time interval between the last activity and now, this
	 * <tt>Channel</tt>'s transport is considered inactive.
	 */
	 ...
	 * <tt>Channel</tt>. Currently this means when for the last time there were any
	 * RTP/RTCP packets received for this channel.
	 */
	...
	/**
	 * The <tt>TransportManager</tt> that represents the Jingle transport of this
	 * <tt>Channel</tt>.
	 */
	 ...
	 *                           <tt>AudioChannel</tt> is to be a part of (or
	...
	/**
	 * Called when this {@code Channel} is being expired. Extenders should close any
	 * open streams.
	 *
	 * @throws IOException if an I/O error occurs while closing any open streams.
	 ...
	 *
	 * @return a new <tt>StreamConnector</tt> instance which represents the pair of
	 *         <tt>DatagramSocket</tt>s for RTP and RTCP traffic
	...
	/**
	 * Initializes a <tt>MediaStreamTarget</tt> instance which identifies the remote
	 * addresses to transmit RTP and RTCP to and from.
	 *
	 * @return a <tt>MediaStreamTarget</tt> instance which identifies the remote
	 *         addresses to transmit RTP and RTCP to and from
	 */
	...
	/**
	 * Initializes a new <tt>TransportManager</tt> instance which has a specific XML
	 * namespace.
	 *
	 * @param xmlNamespace the XML namespace of the new <tt>TransportManager</tt>
	 *                     instance to be initialized
	 * @return a new <tt>TransportManager</tt> instance which has the specified
	 *         <tt>xmlNamespace</tt>
	 * @throws IOException if an error occurs during the initialization of the new
	 *                     <tt>TransportManager</tt> instance which has the
	 *                     specified <tt>xmlNamespace</tt>
	 */
	protected TransportManager createTransportManager(String xmlNamespace) throws IOException {
		if (IceUdpTransportPacketExtension.NAMESPACE.equals(xmlNamespace)) {
			...
			return new IceUdpTransportManager(content.getConference(), isInitiator(), 2 /* numComponents */,
					content.getName());
	 ...
	 *
	 * @param iq the <tt>ColibriConferenceIQ.Channel</tt> on which to set the values
	 ...
	 *
	 * @param iq the <tt>ColibriConferenceIQ.Channel</tt> on which to set the values
	 *           of the properties of <tt>transportManager</tt>
	 */
	 ...
	 * instance throughout its life time and prepares it to be garbage collected.
	 * 
	 * @return {@code true} if the channel was expired as a result of this call, and
	 *         {@code false} if the channel was already expired.
	 */
	public boolean expire() {
		...
		try {
			...
			try {
				...
				logger.warn("Failed to close the MediaStream/stream of channel " + getID() + " of content "
						+ content.getName() + " of conference " + conference.getID() + "!", t);
			...
			try {
				...
				logger.warn("Failed to close the TransportManager/transportManager" + " of channel " + getID()
						+ " of content " + content.getName() + " of conference " + conference.getID() + "!", t);
				if (t instanceof ThreadDeath) {
					throw (ThreadDeath) t;
				}
			}
			...
			if (logger.isInfoEnabled()) {
				...
				logger.info(Logger.Category.STATISTICS, "expire_ch," + getLoggingId());
			}
		}
	...
	/**
	 * Gets the <tt>BundleContext</tt> associated with this <tt>Channel</tt>. The
	 * method is a convenience which gets the <tt>BundleContext</tt> associated with
	 * the XMPP component implementation in which the <tt>Videobridge</tt>
	 ...
	 * created.
	 *
	 * @return the time in milliseconds which indicates when this <tt>Channel</tt>
	...
	/**
	 * Child classes should implement this method and return <tt>DtlsControl</tt>
	 * instance if they are willing to use DTLS transport. Otherwise, <tt>null</tt>
	 * should be returned.
	 *
	...
	/**
	 * Gets the number of seconds of inactivity after which this <tt>Channel</tt>
	 * expires.
	 *
	 * @return the number of seconds of inactivity after which this <tt>Channel</tt>
	 *         expires
	 */
	 ...
	 * there as well).
	 *
	 * @return the ID of this <tt>Channel</tt> (which is unique within the list of
	 *         <tt>Channel</tt> listed in {@link #content} while this instance is
	...
	/**
	 * Gets the time in milliseconds of the last payload related activity for this
	 * <tt>Channel</tt>.
	 *
	 * @return the time in milliseconds of the last payload related activity for
	 *         this <tt>Channel</tt>.
	 *
	...
	/**
	 * Gets the time in milliseconds of the last transport related activity for this
	 * <tt>Channel</tt>.
	 *
	 * @return the time in milliseconds of the last transport related activity for
	 *         this <tt>Channel</tt>.
	 *
	 * @see #lastTransportActivityTime
	 */
	 ...
	 * {@link #transportNamespace} or obtains instance from {@link Conference} if
	...
	void initialize() throws IOException {
		synchronized (transportManagerSyncRoot) {
			...
			if (channelBundleId == null) {
				transportManager = createTransportManager(transportNamespace);
			}
			// Otherwise, it uses a TransportManager specific to the
			// channel-bundle, which is maintained by the Conference object.
			else {
				transportManager = getContent().getConference().getTransportManager(channelBundleId, true,
						isInitiator());
			}
	...
	/**
	 * Gets the indicator which determines whether {@link #expire()} has been called
	 ...
	 * negotiation associated with this instance.
	 *
	 * @return <tt>true</tt> if the conference focus is the initiator/offerer (as
	 *         opposed to the responder/answerer) of the media negotiation
	...
	/**
	 * Starts this channel's stream if it has not been started yet and if the state
	 * of this <tt>Channel</tt> meets the prerequisites to invoke
	 ...
	 * {@link #transportManager} has connected.
	 *
	...
	/**
	 * Sets the {@link Endpoint} of this {@link Channel} to a particular instance.
	 * 
	...
	/**
	 * Sets the number of seconds of inactivity after which this <tt>Channel</tt> is
	 * to expire.
	 *
	 ...
	 *                  initiator/offerer (as opposed to the responder/answerer) of
	 *                  the media negotiation associated with this instance;
	...
	public void setInitiator(boolean initiator) {
		...
		if (oldValue != newValue) {
			/*
			 * TODO Handle the change of initiator? Or remove the functionality on
			 * channel-level?
			 * 
			 * DtlsControl dtlsControl = getDtlsControl();
			 * 
			 * if(dtlsControl != null) { dtlsControl.setSetup( isInitiator() ?
	...
	public void setTransport(IceUdpTransportPacketExtension transport) {
		if (transport != null) {
			...
			if (transportManager != null) {
				...
				logger.warn("Failed to start connectivity establishment: " + "transport manager is null.");
			}
		}
	 ...
	 *
	 * @param activityType the activity type that has happened on this channel.
	 */
	...
	/**
	 * Notifies this <tt>Channel</tt> that its <tt>TransportManager</tt> has been
	 * closed.
	 */
	...
	void transportConnected() {
		logger.info(Logger.Category.STATISTICS, "transport_connected," + getLoggingId());
		...
		try {
			...
			logger.warn("Failed to start stream for channel: " + getID() + ": " + ioe);
		}
	}

	/**
	 * Returns the ID of channel-bundle of this <tt>Channel</tt>, or <tt>null</tt>
	 * if the <tt>Channel</tt> is not part of a channel-bundle.
	 * 
	 * @return the ID of channel-bundle of this <tt>Channel</tt>, or <tt>null</tt>
	 *         if the <tt>Channel</tt> is not part of a channel-bundle.
	 */
	...
	/**
	 * @return a string which identifies this{@link Channel} for the purposes of
	...
	public boolean shouldExpire() {
		return getLastActivityTime() + 1000L * getExpire() < System.currentTimeMillis();
	}
	...
	public enum ActivityType {
		/**
		 * Transport level activity like ICE consent checks and/or RTP/RTCP packets
		 * received.
		 */
		TRANSPORT,
