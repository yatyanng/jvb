...
public class Health extends PeriodicRunnableWithObject<Videobridge> {
	/**
	 * The {@link Logger} used by the {@link Health} class and its instances to
	 ...
	 * {@link Videobridge}. For example, {@link MediaType#DATA} is not supported by
	 * {@link Content#createRtpChannel(String, String, Boolean, RTPLevelRelayType)}.
	 */
	...
	/**
	 * The name of the property which configures the interval between health checks.
	 */
	...
	/**
	 * The name of the property which configures the timeout for health checks. The
	 * {@link #check()} API will return failure unless a there was a health check
	 * performed in the last that many milliseconds.
	 */
	 ...
	 * specific {@link Conference}. The specified {@code conference} will be used to
	 * perform the check i.e. for testing purposes.
	 *
	 ...
	 *                   the {@code videobridge} associated with {@code conference}
	 *                   or the check determines that the {@code Videobridge} is not
	 *                   healthy
	 */
	private static void check(Conference conference) throws Exception {
		...
		for (int i = 0; i < endpoints.length; ++i) {
			...
			for (MediaType mediaType : MEDIA_TYPES) {
				...
				// RtpChannel
				RtpChannel rtpChannel = content.createRtpChannel(channelBundleId, /* transportNamespace */ null,
						initiator, null);
			...
			SctpConnection sctpConnection = dataContent.createSctpConnection(endpoint, /* sctpPort */ RANDOM.nextInt(),
	 ...
	 *
	 * @param videobridge the {@code Videobridge} to check the health (status) of
	 * @throws Exception if an error occurs while checking the health (status) of
	 *                   {@code videobridge} or the check determines that
	 *                   {@code videobridge} is not healthy
	 */
	private static void doCheck(Videobridge videobridge) throws Exception {
		...
		// Conference
		Conference conference = videobridge.createConference(/* focus */ null, /* name */ null,
				/* enableLogging */ false, /* gid */ null);
	...
	/**
	 * Checks if this {@link Videobridge} has an XMPP component and its connection
	 * is alive. Throws an exception if this isn't the case. an XMPP component,
	 *
	 ...
	 *
	 * @param channel the {@code Channel} whose {@code TransportManager} is to be
	 *                represented as a {@code IceUdpTransportPacketExtension}
	...
	private static IceUdpTransportPacketExtension describeTransportManager(Channel channel) {
		ColibriConferenceIQ.ChannelCommon iq = (channel instanceof SctpConnection)
	...
	/**
	 * Generates a pseudo-random {@code Endpoint} ID which is not guaranteed to be
	 * unique.
	 *
	 ...
	 * consecutive pairs).
	 *
	 * @param endpoints the list of {@code Endpoint}s to connect between themselves
	 ...
	 * videobridge. When the health check is successful, this is {@code null}.
	 */
	...
	/**
	 * The timeout in milliseconds after which this videobridge will be considered
	 * unhealthy; i.e. if no health check has been completed in the last
	 * {@code timeout} milliseconds the bridge is unhealthy.
	 */
	private final int timeout;

	/**
	 * Whether failures are sticky, i.e. once the bridge becomes unhealthy it will
	 * never go back to a healthy state.
	 */
	...
	public Health(Videobridge videobridge, ConfigurationService cfg) {
		...
		int period = cfg == null ? PERIOD_DEFAULT : cfg.getInt(PERIOD_PNAME, PERIOD_DEFAULT);
		setPeriod(period);

		timeout = cfg == null ? TIMEOUT_DEFAULT : cfg.getInt(TIMEOUT_PNAME, TIMEOUT_DEFAULT);

		stickyFailures = cfg == null ? STICKY_FAILURES_DEFAULT
				: cfg.getBoolean(STICKY_FAILURES_PNAME, STICKY_FAILURES_DEFAULT);
	...
	protected void doRun() {
		...
		if (exception == null) {
			logger.info("Performed a successful health check in " + duration + "ms. Sticky failure: "
					+ (stickyFailures && hasFailed));
		} else {
	...
	/**
	 * Checks the health of this {@link Videobridge}. This method only returns the
	 * cache results, it does not do the actual health check (i.e. creating a test
	 * conference).
	 *
	 * @throws Exception if an error occurs while checking the health (status) of
	 *                   {@code videobridge} or the check determines that
	...
	public void check() throws Exception {
		...
		if (timeSinceLastResult > timeout) {
			throw new Exception(
					"No health checks performed recently, the last result was " + timeSinceLastResult + "ms ago.");
		}
