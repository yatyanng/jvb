...
public class RawUdpTransportManager extends TransportManager {
	/**
	 * The {@link Logger} used by the {@link RawUdpTransportManager} class to print
	...
	/**
	 * The <tt>StreamConnector</tt> that represents the datagram sockets allocated
	 * by this instance for the purposes of RTP and RTCP transmission.
	 */
	private final StreamConnector streamConnector;

	/**
	 * Indicates whether this transport manager has been started.
	 * {@link Channel#transportConnected()} call has been delayed to the moment when
	 * we receive first transport info for this channel
	...
	private MediaStreamTarget mediaStreamTarget;

	/**
	 * Initializes a new <tt>RawUdpTransportManager</tt> instance.
	 *
	 * @param channel the <tt>Channel</tt> which is initializing the new instance
	 */
	public RawUdpTransportManager(Channel channel) throws IOException {
		...
		/*
		 * Each candidate harvest modifies the generation and the IDs of the RTP and
		 * RTCP candidates.
		 */
		generation = 0;
	...
	/**
	 * {@inheritDoc} Make sure that {@link #close()} is called.
	 */
	@Override
	 ...
	 * Details <a href="http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=4665037">
	 * here</a>.
	 * <p>
	...
	/**
	 * Allocates the datagram sockets expected of this <tt>TransportManager</tt> for
	 * the purposes of RTCP and RTP transmission and represents them in the form of
	 * a <tt>StreamConnector</tt> instance.
	 *
	 * @return a new <tt>StreamConnector</tt> which represents the datagram sockets
	 *         allocated by this instance for the purposes of RTCP and RTP
	 *         transmission
	...
	private StreamConnector createStreamConnector() throws IOException {
		/*
		 * Determine the local InetAddress the new StreamConnector is to attempt to bind
		 * to.
		 */
		BundleContext bundleContext = channel.getBundleContext();
		NetworkAddressManagerService nams = ServiceUtils.getService(bundleContext, NetworkAddressManagerService.class);
		InetAddress bindAddr = null;

		if (nams != null) {
			/*
			 * If there are any ComponentImpl instances implementing the XMPP API of Jitsi
			 * Videobridge, their domains may be used as a hint to the
			...
			for (ComponentImpl component : videobridge.getComponents()) {
				...
				if ((domain != null) && (domain.length() != 0)) {
					/*
					 * The domain reported by the Jabber component contains its dedicated subdomain
					 * and the goal here is to get the domain of the XMPP server.
					 */
					...
					if (domain.length() != 0) {
						try {
							bindAddr = nams.getLocalHost(NetworkUtils.getInetAddress(domain));
						} catch (UnknownHostException uhe) {
							logger.info("Failed to get InetAddress from " + domain + " for channel " + channel.getID()
									+ " of content " + content.getName() + " of conference " + conference.getID() + ".",
									uhe);
						}
					}
				}
		...
		if (bindAddr == null) {
			...
			if (cfg != null) {
				String localAddressStr = cfg.getString(HarvesterConfiguration.NAT_HARVESTER_LOCAL_ADDRESS);
		...
		/*
		 * Try to follow the convention that the RTCP port is the next one after the RTP
		 * port.
		 */
	 ...
	 * {@inheritDoc}
	 *
	 * Overrides the super implementation to add compatibility with legacy Jitsi and
	 * Jitsi Videobridge.
	 */
	@Override
	...
	protected void describe(IceUdpTransportPacketExtension pe) {
		...
		// RTP
		{
			...
			candidate.setComponent(CandidatePacketExtension.RTP_COMPONENT_ID);
	...
	/**
	 * {@inheritDoc} RawUdpTransportManager does not implement DTLS.
	 */
	@Override
	 ...
	 * <tt>null</tt> because it does not establish connectivity and, consequently,
	 * does not learn the remote addresses and requires latching.
	 */
	@Override
	...
	public void startConnectivityEstablishment(IceUdpTransportPacketExtension transport) {
		...
		if (!(transport instanceof RawUdpTransportPacketExtension)) {
			logger.error("Only RAW transport is accepted by this transport manager.");
			return;
		}
	...
	public MediaStreamTarget getMediaStreamTarget() {
		return mediaStreamTarget;
	}

	public void setMediaStreamTarget(MediaStreamTarget mediaStreamTarget) {
		this.mediaStreamTarget = mediaStreamTarget;
	}
}
