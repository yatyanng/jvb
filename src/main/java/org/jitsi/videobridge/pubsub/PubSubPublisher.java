...
public class PubSubPublisher {
	...
	/**
	 * Gets a <tt>PubSubPublisher</tt> instance for a specific service (name). If a
	 * <tt>PubSubPublisher</tt> instance for the specified <tt>serviceName</tt> does
	 * not exist yet, a new instance is initialized.
	 *
	...
	private void configureNode(String nodeName) {
		...
		pubsub.addExtension(new FormNode(FormNodeType.CONFIGURE_OWNER, nodeName, cfg));
		try {
			send(pubsub);
		} catch (Exception e) {
		...
		timeoutTimer.schedule(new TimerTask() {
			@Override
			public void run() {
				String nodeName = pendingConfigureRequests.remove(packetID);

				if (nodeName != null) {
					logger.error("Timed out a configuration request " + "(packetID=: " + packetID + " nodeName="
							+ nodeName + ")");
					fireResponseCreateEvent(PubSubResponseListener.Response.SUCCESS);
				}
			}
	...
	public void createNode(String nodeName) throws Exception {
		...
		request.addExtension(new NodeExtension(PubSubElementType.CREATE, nodeName));
		...
		timeoutTimer.schedule(new TimerTask() {
			...
			public void run() {
				...
				if (nodeName != null) {
					logger.warn("Timed out a create request with ID " + packetID);
				}
			}
		}, PACKET_TIMEOUT);
	}
	...
	/**
	 * Releases the resources of this <tt>PubSubPublisher</tt> i.e. prepares it for
	 * garbage collection.
	 */
	...
	private void handleErrorResponse(IQ response) {
		...
		if (err != null) {
			...
			if ((XMPPError.Type.CANCEL.equals(errType) && (XMPPError.Condition.conflict.equals(errCondition)
					|| XMPPError.Condition.forbidden.equals(errCondition)))
					/* prosody bug, for backward compatibility */
					|| (XMPPError.Type.AUTH.equals(errType) && XMPPError.Condition.forbidden.equals(errCondition))) {
				...
				String nodeName = pendingCreateRequests.remove(packetID);
				logger.info("PubSub node already exists (packetID=" + packetID + " nodeName=" + nodeName + ")");
		...
		errMsg.append(".");
		if (err != null) {
			errMsg.append(" Message: ").append(err.getDescriptiveText()).append(". Condition: ")
					.append(err.getCondition()).append(". For packet with id: ").append(packetID).append(".");
		}
		logger.error(errMsg);
	}
	 ...
	 * @param itemId   the ID of the item to be published. If <tt>null</tt> the XMPP
	 *                 server will generate random ID by itself.
	...
	public void publish(String nodeName, String itemId, ExtensionElement ext) throws Exception {
		...
		timeoutTimer.schedule(new TimerTask() {
			@Override
			public void run() {
				String nodeName = pendingPublishRequests.remove(packetID);

				if (nodeName != null) {
					logger.error("Timed out a publish request: " + nodeName);
				}
			}
		}, PACKET_TIMEOUT);
		send(packet);
