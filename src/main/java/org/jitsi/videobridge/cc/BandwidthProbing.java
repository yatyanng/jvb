...
public class BandwidthProbing extends PeriodicRunnable {
	/**
	 * The system property name that holds a boolean that determines whether or not
	 * to activate the RTX bandwidth probing mechanism that implements stream
	 * protection.
	 */
	public static final String DISABLE_RTX_PROBING_PNAME = "org.jitsi.videobridge.DISABLE_RTX_PROBING";

	/**
	 * The system property name that holds the interval/period in milliseconds at
	 * which {@link #run()} is to be invoked.
	 */
	 ...
	 * series.
	 */
	private static final TimeSeriesLogger timeSeriesLogger = TimeSeriesLogger
			.getTimeSeriesLogger(BandwidthProbing.class);
	...
	/**
	 * the interval/period in milliseconds at which {@link #run()} is to be invoked.
	 */
	private static final long PADDING_PERIOD_MS = cfg != null ? cfg.getInt(PADDING_PERIOD_MS_PNAME, 15) : 15;
	 ...
	 */
	private static final boolean DISABLE_RTX_PROBING = cfg != null && cfg.getBoolean(DISABLE_RTX_PROBING_PNAME, false);
	 ...
	 * Ctor.
	 *
	 * @param dest the {@link VideoChannel} to probe for available send bandwidth.
	 */
	...
	public void run() {
		...
		if (destStream == null || (destStream.getDirection() != null && !destStream.getDirection().allowsSending())
		...
		List<SimulcastController> simulcastControllerList = dest.getBitrateController().getSimulcastControllers();
		...
		for (SimulcastController simulcastController : simulcastControllerList) {
			...
			long currentBps = sourceTrack.getBps(simulcastController.getCurrentIndex());
			...
			totalTargetBps += sourceTrack.getBps(simulcastController.getTargetIndex());
			totalOptimalBps += sourceTrack.getBps(simulcastController.getOptimalIndex());
		}

		// How much padding do we need?
		long totalNeededBps = totalOptimalBps - Math.max(totalTargetBps, totalCurrentBps);
		if (totalNeededBps < 1) {
			// Not much.
			return;
		}
		...
		if (timeSeriesLogger.isTraceEnabled()) {
			...
			timeSeriesLogger
					.trace(diagnosticContext.makeTimeSeriesPoint("out_padding").addField("padding_bps", paddingBps)
							.addField("optimal_bps", totalOptimalBps).addField("current_bps", totalCurrentBps)
							.addField("target_bps", totalTargetBps).addField("needed_bps", totalNeededBps)
							.addField("max_padding_bps", maxPaddingBps).addField("bwe_bps", bweBps));
		}
		...
		for (int i = 0; i < len; i++) {
			try {
				...
				RawPacket pkt = RawPacket.makeRTP(mediaSSRC, vp8PT, seqNum++, ts, pktLen);
	...
	/**
	 * (attempts) to get the local SSRC that will be used in the media sender SSRC
	 * field of the RTCP reports. TAG(cat4-local-ssrc-hurricane)
	 *
	 * @return get the local SSRC that will be used in the media sender SSRC field
	 *         of the RTCP reports.
	 */
