...
public class CallStatsIOTransport extends StatsTransport {
	...
	/**
	 * The {@code BridgeStatistics} which initializes new {@code BridgeStatusInfo}
	 * instances (to be sent by {@code jitsi-stats}). Since reentrancy and
	 * thread-safety related issues are taken care of by the invoker of
	 * {@link #publishStatistics(Statistics)}, the instance is cached for the sake
	 * of performance.
	 */
	...
	private void dispose(BundleContext bundleContext) {
		...
		if (statsService != null) {
			StatsServiceFactory.getInstance().stopStatsService(bundleContext, statsService.getId());
			statsService = null;
		}
	 ...
	 * {@link #publishStatistics(Statistics)} may executed successfully. Initializes
	 * {@link #statsService} i.e. the jitsi-stats library.
	 *
	 * @param bundleContext the {@code BundleContext} in which this
	...
	private void init(BundleContext bundleContext, ConfigurationService cfg) {
		...
		String appSecret = ConfigUtils.getString(cfg, PNAME_CALLSTATS_IO_APP_SECRET, null);
		String keyId = ConfigUtils.getString(cfg, PNAME_CALLSTATS_IO_KEY_ID, null);
		String keyPath = ConfigUtils.getString(cfg, PNAME_CALLSTATS_IO_KEY_PATH, null);
		String bridgeId = ConfigUtils.getString(cfg, PNAME_CALLSTATS_IO_BRIDGE_ID, DEFAULT_BRIDGE_ID);
	 ...
	 *
	 * @param bsib                the {@code BridgeStatistics} into which data read
	 *                            from {@code statistics} is to be written
	 * @param s                   the {@code Statistics} from which data is to be
	 *                            read and written into
	 *                            {@code bridgeStatusInfoBuilder}
	 * @param measurementInterval the interval of time in milliseconds covered by
	 *                            the measurements carried by the specified
	 *                            {@code statistics}
	 */
	private void populateBridgeStatusInfoBuilderWithStatistics(BridgeStatistics bsib, Statistics s,
			long measurementInterval) {
		bsib.audioFabricCount(s.getStatAsInt(VideobridgeStatistics.AUDIOCHANNELS));
		bsib.avgIntervalJitter(s.getStatAsInt(VideobridgeStatistics.JITTER_AGGREGATE));
		bsib.avgIntervalRtt(s.getStatAsInt(VideobridgeStatistics.RTT_AGGREGATE));
		bsib.conferenceCount(s.getStatAsInt(VideobridgeStatistics.CONFERENCES));
		bsib.cpuUsage((float) s.getStatAsDouble(VideobridgeStatistics.CPU_USAGE));
		bsib.intervalDownloadBitRate((int) Math.round(s.getStatAsDouble(VideobridgeStatistics.BITRATE_DOWNLOAD)));
		...
		bsib.intervalRtpFractionLoss((float) s.getStatAsDouble(VideobridgeStatistics.LOSS_RATE_DOWNLOAD));
		// TODO intervalSentBytes
		bsib.intervalUploadBitRate((int) Math.round(s.getStatAsDouble(VideobridgeStatistics.BITRATE_UPLOAD)));
		bsib.measurementInterval((int) measurementInterval);
		bsib.memoryUsage(s.getStatAsInt(VideobridgeStatistics.USED_MEMORY));
		bsib.participantsCount(s.getStatAsInt(VideobridgeStatistics.NUMBEROFPARTICIPANTS));
		...
		bsib.videoFabricCount(s.getStatAsInt(VideobridgeStatistics.VIDEOCHANNELS));
	}
