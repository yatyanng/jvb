...
public class StatsManager extends BundleContextHolder2 {
	 ...
	 */
	private final RecurringRunnableExecutor statisticsExecutor = new RecurringRunnableExecutor(
	 ...
	 * {@link #transports}.
	 */
	private final RecurringRunnableExecutor transportExecutor = new RecurringRunnableExecutor(
			StatsManager.class.getSimpleName() + "-transportExecutor");

	/**
	 * The <tt>StatsTransport</tt>s added to this <tt>StatsManager</tt> to transport
	 * {@link #statistics}.
	 */
	 ...
	 *                   generated/updated by this <tt>StatsManager</tt> at the
	 *                   specified <tt>period</tt>
	 ...
	 * <tt>StatsManager</tt> is to periodically send the <tt>Statistics</tt> added
	 * to it.
	 * <p>
	 * Warning: {@code StatsTransport}s added to this {@code StatsManager} after
	 * {@link #start(BundleContext)} has been invoked will not be called.
	 * </p>
	 *
	 * @param transport the <tt>StatsTransport</tt> to add to this
	 *                  <tt>StatsManager</tt> so that the latter periodically sends
	 *                  the <tt>Statistics</tt> added through the former
	 * @param period    the internal/period in milliseconds at which this
	 *                  <tt>StatsManager</tt> is to repeatedly send the
	 *                  <tt>Statistics</tt> added to it through the specified
	 *                  <tt>transport</tt>
	 */
	...
	/**
	 * Finds the first instance of {@code Statistics} with a specific runtime type
	 * generated/updated at a specific interval/period.
	 *
	 ...
	 *         {@code clazz} generated/updated every {@code period} milliseconds if
	 ...
	 *
	 * @return a <tt>Collection</tt> of the (sets of) <tt>Statistics</tt> which this
	 ...
	 * generates/updates.
	 *
	 * @return a <tt>Collection</tt> of the <tt>StatTransport</tt>s through which
	 *         this <tt>StatsManager</tt> periodically sends the (sets of)
	 ...
	 * {@inheritDoc}
	 *
	 * Starts the {@code StatsTransport}s added to this {@code StatsManager} in the
	 * specified {@code bundleContext}. Commences the generation of the
	 ...
	 * {@inheritDoc}
	 *
	 * Stops the <tt>StatsTransport</tt>s added to this <tt>StatsManager</tt> in the
	...
	private static class StatisticsPeriodicRunnable extends PeriodicRunnableWithObject<Statistics> {
		/**
		 * Initializes a new {@code StatisticsPeriodicRunnable} instance which is to
		 * {@code period}ically generate {@code statistics}.
		 *
		 * @param statistics the {@code Statistics} to be {@code period}ically generated
		 *                   by the new instance
		 * @param period     the time in milliseconds between consecutive generations of
		 *                   {@code statistics}
		 */
	...
	private class TransportPeriodicRunnable extends PeriodicRunnableWithObject<StatsTransport> {
		 ...
		 * {@code period}ically generate {@code statistics}.
		 *
		 * @param transport the {@code StatsTransport} to {@code period}ically publish
		 *                  {@link #statistics} to
		 * @param period    the time in milliseconds between consecutive invocations of
		 *                  {@code publishStatistics()} on {@code transport}
		 */
