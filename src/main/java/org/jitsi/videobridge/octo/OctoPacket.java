 ...
 * </pre>
 * 
 * R: Source-is-a-relay flag. 1 if the source of the packet is a relay, and 0 if
 * it is an endpoint (bridge).
 * <p/>
...
public class OctoPacket {
	...
	/**
	 * @return the integer used to identify the particular {@link MediaType} in
	 *         Octo.
	 */
	 ...
	 */
	public static void writeHeaders(byte[] buf, int off, boolean r, MediaType mediaType, int s, String conferenceId,
			String endpointId) {
		buf[off] = 0;
		if (r) {
			buf[off] |= 0x80;
	 ...
	 * @param buf          the buffer which contains the Octo header.
	 * @param off          the offset in {@code buf} at which the Octo header
	 *                     begins.
	 * @param len          the length of the buffer.
	 */
	...
	/**
	 * Verifies that a given buffer has the minimum length of a valid Octo packet.
	 * 
	 ...
	 * {@code len} has an effective length of at least {@code minLength}. packet.
	 * 
	 * @param buf the {@code byte[]} which contains the buffer.
	 * @param off the offset at which the buffer starts.
	 * @param len the length of the buffer.
