...
/**
 * An {@link CsrcAudioLevelListener} implementation which is associated with a
 * particular {@link RtpChannel} and forwards received audio levels to the
...
public class AudioChannelAudioLevelListener implements CsrcAudioLevelListener {
	 ...
	 * {@link RtpChannel}.
	 *
	 * @param levels a <tt>long</tt> array in which the elements at the even indices
	 *               specify the CSRC IDs and the elements at the odd indices
	...
	public void audioLevelsReceived(long[] levels) {
		if (levels != null) {
			/*
			 * Forward the audio levels of the contributors to this Channel to the
			 * active/dominant speaker detection/identification algorithm.
			 */
			int[] receiveSSRCs = channel.getReceiveSSRCs();

			if (receiveSSRCs.length != 0) {
				/*
				 * The SSRCs are at the even indices, their audio levels at the immediately
				 * subsequent odd indices.
				 */
				for (int i = 0, count = levels.length / 2; i < count; i++) {
					int i2 = i * 2;
					long ssrc = levels[i2];
					/*
					 * The contributing SSRCs may not all be from sources associated with this
					 * Channel and we're only interested in the latter here.
					 */
					...
					if (isReceiveSSRC) {
						ConferenceSpeechActivity conferenceSpeechActivity = channel.conferenceSpeechActivity;
