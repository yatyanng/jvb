...
public abstract class Statistics {
	...
	public static ColibriStatsIQ toXmppIq(Statistics statistics) {
		...
		for (Map.Entry<String, Object> e : statistics.getStats().entrySet()) {
			iq.addStat(new ColibriStatsExtension.Stat(e.getKey(), e.getValue()));
		}
		return iq;
	}
	 ...
	 * modification of the state of this instance. Replaces <tt>synchronized</tt>
	 * blocks in order to reduce the number of exclusive locks and, therefore, the
	 * risks of superfluous waiting.
	 */
	...
	/**
	 * Gets the value of a specific piece of statistic as a {@code double} value.
	 *
	...
	/**
	 * Sets the value of a specific piece of statistics. The method assumes that the
	 * caller has acquired the write lock of {@link #lock} and, thus, allows the
	 * optimization of batch updates to multiple pieces of statistics.
	 *
