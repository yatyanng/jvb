...
public class OctoTransportManager extends TransportManager {
	/**
	 * The {@link Logger} used by the {@link OctoTransportManager} class to print
	...
	/**
	 * Converts a "relay ID" to a socket address. The current implementation assumes
	 * that the ID has the form of "address:port".
	 * 
	 ...
	 * socket, which receives only RTP packets destined for this transport manager
	 * (or rather its {@link OctoChannel}.
	 */
	private DatagramSocket rtpSocket;

	/**
	 * The {@link MultiplexedDatagramSocket} derived from the {@link OctoRelay}'s
	 * socket, which receives only RTCP packets destined for this transport manager
	 * (or rather its {@link OctoChannel}.
	 */
	private DatagramSocket rtcpSocket;

	/**
	 * The (non-functional) {@link SrtpControl} maintained by this instance, which
	 * is used as the {@link SrtpControl} of the {@link MediaStream} of the channel.
	 * It is necessary to use a {@link NullSrtpControl}, because {@link MediaStream}
	 * defaults to a ZRTP implementation otherwise.
	 */
	private final SrtpControl srtpControl = new NullSrtpControl();

	/**
	 * The list of addresses of remote relays to which outgoing packets should be
	 * sent. This list is maintained by the conference organizer based on the
	 * videobridges participating in the conference, and configured to this
	...
	public OctoTransportManager(Channel channel) {
		...
		logger = Logger.getLogger(classLogger, channel.getContent().getConference().getLogger());

		OctoRelayService relayService = ServiceUtils.getService(channel.getBundleContext(), OctoRelayService.class);
	...
	public StreamConnector getStreamConnector(Channel channel) {
		...
		return new DefaultStreamConnector(rtpSocket, rtcpSocket, true /* rtcpmux */);
	}

	/**
	 * Initializes the filtered sockets derived from the socket maintained by the
	 * {@link OctoRelay}, which are to receive RTP (or RTCP) packets from the
	 * {@link OctoChannel} associated with this transport manager.
	 * 
	 * @throws IOException
	 */
	private void initializeSockets() throws IOException {
		synchronized (socketsSyncRoot) {
			...
			rtpSocket = createOctoSocket(relaySocket.getSocket(channel.getDatagramFilter(false)));
			rtcpSocket = createOctoSocket(relaySocket.getSocket(channel.getDatagramFilter(true)));
		}
	}

	/**
	 * Creates a {@link DatagramSocket} instance which wraps a specific other
	 * {@link DatagramSocket} instance for the purposes of Octo. Specifically, it
	 * overrides {@code receive()} to strip the Octo headers, and overrides
		...
		DatagramSocket s = new DelegatingDatagramSocket(socket) {
			...
			public void receive(DatagramPacket p) throws IOException {
				...
				try {
					...
					// associated channel.
					p.setData(p.getData(), p.getOffset() + OctoPacket.OCTO_HEADER_LENGTH,
							p.getLength() - OctoPacket.OCTO_HEADER_LENGTH);
				} catch (Exception e) {
					// TODO: more graceful handling
					logger.error("Failed to strip Octo header while receiving a packet:" + e);
				}
			}
	...
	public void setRelayIds(List<String> relayIds) {
		...
		for (String relayId : relayIds) {
			...
			if (socketAddress == null) {
				logger.error("Could not convert a relay ID to a socket address: " + relayId);
				continue;
			}
	 ...
	 * {@link TransportManager} (i.e. to all {@link #remoteRelays}). The payload of
	 * the packet is first encapsulated in Octo (i.e. the fixed size Octo header is
	 * prepended).
	 *
	 * @param p          the packet to send.
	 * @param addHeaders whether this call should add the Octo headers to the packet
	 *                   before sending or not.
	 * @throws IOException if calling {@code send()} on any of the target sockets
	...
	private void doSend(DatagramPacket p, boolean addHeaders) throws IOException {
		...
		if (exception != null) {
			logger.warn("Caught " + exceptions + " while trying to send a packet.");
			throw exception;
		}
	}
	...
	private DatagramPacket addOctoHeaders(DatagramPacket p) {
		...
		OctoPacket.writeHeaders(buf, off, true /* source is a relay */, channel.getMediaType(),
				0 /* simulcast layers info */, channel.getConferenceId(),
	...
	void sendMessage(String msg, String sourceEndpointId, String conferenceId) {
		...
		System.arraycopy(msgBytes, 0, buf, OctoPacket.OCTO_HEADER_LENGTH, msgBytes.length);

		OctoPacket.writeHeaders(buf, 0, true /* source is a relay */, MediaType.DATA, 0 /* simulcast layers info */,
				conferenceId, sourceEndpointId);
