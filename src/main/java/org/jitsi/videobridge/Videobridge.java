...
public class Videobridge
{
    ...
    */
            ...
            int options)
    {
        ...
        logger.debug("PHASE 3: [" + conferenceIQ.getStanzaId()
        	+ "] Number of Contents is "+conferenceIQ.getContents().size());
                ...
                : conferenceIQ.getContents())
        {
                    ...
                    : contentIQ.getChannels())
            {
                ...
                logger.debug("PHASE 3A2: ["+conferenceIQ.getStanzaId()+"] Channel ID: "+channelID
                		+", Channel Expire: "+channelExpire+", End Point ID: " +endpointID
                		+", Transport: "+channelIQ.getTransport());
                ...
                if (channelTransportManager instanceof RawUdpTransportManager) {
                	...
                	if (channelIQ.getTransport() != null) {
                    	...
                    	for (CandidatePacketExtension candidate : candidates) {
                    		logger.debug("PHASE 3A3:  ["+conferenceIQ.getStanzaId()+"] component["+candidate.getComponent()
                    			+"]: "+candidate.getIP()+":"+candidate.getPort());
                    		
                    		if (candidate.getComponent() == org.ice4j.ice.Component.RTP) {
                    			streamTargetAddresses[0] = new InetSocketAddress(candidate.getIP(), candidate.getPort());
                    			streamTargetAddressCount++;
                    		}
                    		else if (candidate.getComponent() == org.ice4j.ice.Component.RTCP) {
                    			streamTargetAddresses[1] = new InetSocketAddress(candidate.getIP(), candidate.getPort());
                    			streamTargetAddressCount++;
                    		}
                    	}
                	...
                	logger.debug("PHASE 3A3: ["+conferenceIQ.getStanzaId()+"] Channel Transport Manager is not of type "
                			+RawUdpTransportManager.class.getName()+", End Point ID: " +endpointID);
                }
                        ...
                        != ColibriConferenceIQ.Channel.EXPIRE_NOT_SPECIFIED)
                {
                    ...
                    if ((channelExpire == 0) && channel.isExpired())
                    {
                    	if (endpointID == null) {
                    		...
                    		logger.debug("PHASE 3A4: ["+conferenceIQ.getStanzaId()+"] End Point becomes "+endpointID);
                    	}
                    	conference.getOrCreateEndpoint(endpointID).expire();
                    	logger.debug("PHASE 3A4: ["+conferenceIQ.getStanzaId()+"] End point Count: "+conference.getEndpointCount());
                    	if (conference.getEndpointCount() <= 1) {
                    		conference.safeExpire();
                    		logger.debug("PHASE 3A4: ["+conferenceIQ.getStanzaId()+"] Expired Conference: "+conference.getID());
                    	}
                        continue;
                    }
                }
                    ...
                    : contentIQ.getSctpConnections())
            {
                ...
                if (endpointID != null)
                {
                	logger.debug("PHASE 3B2: ["+conferenceIQ.getStanzaId()+"] Setting End Point "
                			+endpointID+" to "+sctpConn);
        ...
        {
        	logger.debug("PHASE 5: ["+conferenceIQ.getStanzaId()+"] Updating "
        			+colibriEndpoint.getId()+" in Conference: "+conference.getID());
        ...
        logger.debug("PHASE 5: ["+conferenceIQ.getStanzaId()+"] Endpoint Count is "
        		+conference.getEndpointCount()+" in Conference: "+conference.getID());
