...
public class WebRtcDataStream {
	 ...
	 * The logger
	 */
	private final static Logger logger = Logger.getLogger(WebRtcDataStream.class);
	 ...
	 * @param acknowledged indicates if this channel has been already acknowledged
	 *                     by remote peer.
	 */
	...
	/**
	 * Returns SCTP stream id on which this <tt>WebRtcDataStream</tt> is running.
	 * 
	 * @return SCTP stream id on which this <tt>WebRtcDataStream</tt> is running.
	 */
	public int getSid() {
		return sid;
	}
	...
	public void sendBinary(byte[] bytes) throws IOException {
		int res = socket.send(bytes, true, sid, SctpConnection.WEB_RTC_PPID_BIN);
		if (res != bytes.length) {
			throw new IOException("Failed to send the data");
		}
	}
	...
	/**
	 * Interface used to receive data on this stream. It is message oriented and
	 * supports text or binary payload.
	 */
	public interface DataCallback {
		 ...
		 *
		 * @param src the <tt>WebRtcDataStream</tt> on which this message was received.
		...
		/**
		 * Fired when binary message is received on this <tt>WebRtcDataStream</tt>.
		 *
		 * @param src  the <tt>WebRtcDataStream</tt> on which this message was received.
