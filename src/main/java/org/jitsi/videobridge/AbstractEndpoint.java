...
/**
 * Represents an endpoint in a conference (i.e. the entity associated with a
 * participant in the conference, which connects the participant's audio and
 * video channel). This might be an endpoint connected to this instance of
...
public abstract class AbstractEndpoint extends PropertyChangeNotifier {
	 ...
	 * 
	 * @param conference the {@link Conference} which this endpoint is to be a part
	 *                   of.
	...
	/**
	 * @return the {@link AbstractEndpointMessageTransport} associated with this
	 *         endpoint.
	 */
	...
	/**
	 * Adds a specific {@link RtpChannel} to the list of channels associated with
	 * this {@link AbstractEndpoint}.
	 *
	 * @param channel the {@link RtpChannel }to add.
	 * @return {@code true} if the list of {@link RtpChannel}s associated with this
	 *         endpoint changed as a result of the method invocation; otherwise,
	 *         {@code false}.
	 */
	...
	/**
	 * Gets the number of {@link RtpChannel}s of this endpoint which, optionally,
	 * are of a specific {@link MediaType}.
	 *
	 * @param mediaType the {@link MediaType} of the {@link RtpChannel}s to count or
	 *                  {@code null} to count all {@link RtpChannel}s.
	 ...
	 * @return a <tt>List</tt> with the channels of this <tt>Endpoint</tt> with a
	 *         particular <tt>MediaType</tt>.
	 */
	public List<RtpChannel> getChannels(MediaType mediaType) {
		...
		synchronized (this.channels) {
			for (Iterator<WeakReference<RtpChannel>> i = this.channels.iterator(); i.hasNext();) {
				RtpChannel c = i.next().get();

				if ((c == null) || c.isExpired()) {
					i.remove();
					removed = true;
				} else if ((mediaType == null) || mediaType.equals(c.getContent().getMediaType())) {
					channels.add(c);
				}
			}
		}
	 ...
	 * @return <tt>true</tt> if the list of <tt>Channel</tt>s associated with this
	 *         <tt>Endpoint</tt> changed as a result of the method invocation;
	...
	/**
	 * @return a string which identifies this {@link Endpoint} for the purposes of
	...
	/**
	 * @return the {@link Set} of pinned endpoints, represented as a set of endpoint
	 *         IDs.
	 */
	 ...
	 *
	 * @param mediaType the media type of the {@link MediaStreamTrackDesc} to get.
	 * @return an array that contains all the {@link MediaStreamTrackDesc} of the
	 *         specified media type associated with this {@link Endpoint}, or null.
	 */
	public MediaStreamTrackDesc[] getMediaStreamTracks(MediaType mediaType) {
		return getAllMediaStreamTracks(mediaType).toArray(new MediaStreamTrackDesc[0]);
	}
	...
	protected List<MediaStreamTrackDesc> getAllMediaStreamTracks(MediaType mediaType) {
		...
		channels.stream().map(channel -> channel.getStream().getMediaStreamTrackReceiver()).filter(Objects::nonNull)
	...
	/**
	 * Notify this endpoint that another endpoint has set it as a 'selected'
	 * endpoint, meaning its HD stream has another consumer.
	 */
	...
	/**
	 * Notify this endpoint that another endpoint has stopped consuming its HD
	 * stream.
	 */
