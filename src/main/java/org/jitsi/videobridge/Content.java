...
public class Content extends PropertyChangeNotifier implements RTPTranslator.WriteFilter, Expireable {
	...
	/**
	 * @return a string which identifies a specific {@link Content} for the purposes
	 *         of logging (i.e. includes the name of the content and ID of its
	 *         conference). The string is a comma-separated list of "key=value"
	 *         pairs.
	 ...
	 * <tt>Content</tt>, which is to to be pre-announced by the <tt>Channel</tt>s of
	 * this <tt>Content</tt>.
	 *
	...
	/**
	 * The <tt>MediaType</tt> of this <tt>Content</tt>. The implementation detects
	 * the <tt>MediaType</tt> by looking at the {@link #name} of this instance.
	 */
	...
	/**
	 * Path to the directory into which files relating to media recording for this
	 * <tt>Content</tt> will be stored.
	 *
	 * @deprecated remove-with-recording
	 */
	@Deprecated
	...
	/**
	 * The <tt>RTPTranslator</tt> which forwards the RTP and RTCP traffic between
	 * those {@link #channels} which use a translator as their RTP-level relay.
	 */
	...
	/**
	 * Implements
	 * {@link RTPTranslator.WriteFilter#accept( MediaStream, RawPacket, MediaStream, boolean)}
	 */
	@Override
	 ...
	 * <tt>RtpChannel</tt>s of this <tt>Content</tt>. The new <tt>RtpChannel</tt>
	 * instance has an ID which is unique within the list of <tt>RtpChannel</tt>s of
	 * this <tt>Content</tt>.
	 *
	 * @param channelBundleId    the ID of the channel-bundle that the created
	 *                           <tt>RtpChannel</tt> is to be a part of (or
	 *                           <tt>null</tt> if it is not to be a part of a
	 *                           channel-bundle).
	 * @param transportNamespace transport namespace that will used by new channel.
	 *                           Can be either
	 ...
	 * @throws Exception
	 */
	public RtpChannel createRtpChannel(String channelBundleId, String transportNamespace, Boolean initiator,
			RTPLevelRelayType rtpLevelRelayType) throws Exception {
		return createRtpChannel(channelBundleId, transportNamespace, initiator, rtpLevelRelayType, false /* octo */);
	}
	 ...
	 */
	public RtpChannel createRtpChannel(String channelBundleId, String transportNamespace, Boolean initiator,
			RTPLevelRelayType rtpLevelRelayType, boolean octo) throws IOException {
		RtpChannel channel;
		...
		// Initialize channel
		logger.info("Initializing " + channel.getClass().getName() + " " + channel.hashCode());
		...
		if (logger.isInfoEnabled()) {
			...
			if (octo) {
				...
				transport = "default";
			} else if (IceUdpTransportPacketExtension.NAMESPACE.equals(transportNamespace)) {
				transport = "ice";
			} else if (RawUdpTransportPacketExtension.NAMESPACE.equals(transportNamespace)) {
				transport = "rawudp";
			}

			logger.info(Logger.Category.STATISTICS,
					"create_channel," + channel.getLoggingId() + " transport=" + transport + ",bundle="
							+ channelBundleId + ",initiator=" + initiator + ",media_type=" + getMediaType()
	 ...
	 * @param channelBundleId the ID of the channel-bundle that the created
	 *                        <tt>SctpConnection</tt> is to be a part of (or
	 *                        <tt>null</tt> if it is not to be a part of a
	 *                        channel-bundle).
	 * @param initiator       the value to use for the initiator field, or
	 *                        <tt>null</tt> to use the default value.
	 ...
	 *                     initialize.
	 */
	public SctpConnection createSctpConnection(AbstractEndpoint endpoint, int sctpPort, String channelBundleId,
	...
	/**
	 * Expires this <tt>Content</tt> and its associated <tt>Channel</tt>s. Releases
	 * the resources acquired by this instance throughout its life time and prepares
		...
		try {
			...
			for (Channel channel : getChannels()) {
				try {
					...
					logger.warn("Failed to expire channel " + channel.getLoggingId(), t);
	 ...
	 * specified <tt>channel</tt> is not in the list of <tt>Channel</tt>s of this
	 * <tt>Content</tt>, does nothing).
	 *
	 * @param channel the <tt>Channel</tt> to be expired by this <tt>Content</tt>
	 */
	...
	/**
	 * If media recording is started, finds all SSRCs received on all channels, and
	 * sets their endpoints to the <tt>Recorder</tt>'s <tt>Synchronizer</tt>
	 * instance.
	 *
	 * @deprecated remove-with-recording
	 */
	 ...
	 * <tt>receiveSSRC</tt> in its list of received SSRCs, or <tt>null</tt> in case
	 * no such <tt>Channel</tt> exists.
	 *
	 * @param receiveSSRC the SSRC to search for.
	 * @return a <tt>Channel</tt> of this <tt>Content</tt>, which has
	 *         <tt>receiveSSRC</tt> in its list of received SSRCs, or <tt>null</tt>
	 *         in case no such <tt>Channel</tt> exists.
	 */
	...
	/**
	 * Gets the <tt>BundleContext</tt> associated with this <tt>Content</tt>. The
	...
	/**
	 * Gets the number of <tt>Channel</tt>s of this <tt>Content</tt> that are not
	 * expired.
	 *
	 * @return the number of <tt>Channel</tt>s of this <tt>Content</tt> that are not
	 *         expired.
	 */
	public int getChannelCount() {
		return (int) getChannels().stream().filter(c -> c != null && !c.isExpired()).count();
	}
	 ...
	 *
	 * @return the <tt>Conference</tt> which has initialized this <tt>Content</tt>
	 */
	...
	/**
	 * Returns the local synchronization source identifier (SSRC) associated with
	 * this <tt>Content</tt>,
	 *
	 * @return the local synchronization source identifier (SSRC) associated with
	 *         this <tt>Content</tt>,
	 */
	...
	public MediaDevice getMixer() {
		if (mixer == null) {
			...
			MediaDevice device = MediaType.AUDIO.equals(mediaType) ? new AudioSilenceMediaDevice() : null;

			if (device == null) {
				throw new UnsupportedOperationException(
						"The mixer type of RTP-level relay is not supported" + " for " + mediaType);
			} else {
	 ...
	 * between the <tt>Channel</tt>s of this <tt>Content</tt> which use a translator
	 * as their RTP-level relay.
	 *
	 * @return the <tt>RTPTranslator</tt> which forwards the RTP and RTCP traffic
	...
	public RTPTranslator getRTPTranslator() {
		synchronized (rtpLevelRelaySyncRoot) {
			/*
			 * The expired field of Content is initially assigned true and the only possible
			 * change of the value is from true to false, never from false to true.
			 * Moreover, an existing rtpTranslator will be disposed after the change of
			 * expired from true to false. Consequently, no synchronization with respect to
			 * the access of expired is required.
			 */
			if ((rtpTranslator == null) && !expired) {
				...
				if (rtpTranslator != null) {
					...
					if (rtpTranslator instanceof RTPTranslatorImpl) {
						 ...
						 *
						 * 1. By using the initialLocalSSRC as the SSRC of the translator aren't we
						 * breaking the mixing functionality? because FMJ is going to use its "own" SSRC
						 * to for mixed stream, which remains unannounced.
						 *
						 * 2. By using an initialLocalSSRC we're losing the FMJ collision detection
						 * mechanism.
						 *
						 * The places that are involved in this have been tagged with
	 ...
	 *
	 * @param id the <tt>id</tt> of <tt>SctpConnection</tt> that we're looking for.
	...
	/**
	 * Attempts to enable or disable media recording for this <tt>Content</tt> and
	 * updates the recording path.
	 *
	 ...
	 *
	 * @return the state of the media recording for this <tt>Content</tt> after the
	 *         attempt to enable (or disable).
	 *
	 * @deprecated remove-with-recording
	 */
	@Deprecated
	...
	/**
	 * @return a string which identifies this {@link Content} for the purposes of
	 *         logging (i.e. includes the name of the content and ID of its
	 *         conference). The string is a comma-separated list of "key=value"
	 *         pairs.
	 */
	String getLoggingId() {
