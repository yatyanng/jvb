...
public class OctoChannel extends RtpChannel {
	...
	/**
	 * The Octo ID of the conference, configured in {@link Conference} as the global
	 * ID ({@link Conference#getGid()}).
	 */
	...
	/**
	 * The {@link org.ice4j.socket.DatagramPacketFilter} which (only) accepts RTP
	 * packets for this channel.
	 */
	private RtpChannelDatagramFilter rtpFilter = new OctoDatagramPacketFilter(false /* rtcp */);

	/**
	 * The {@link org.ice4j.socket.DatagramPacketFilter} which (only) accepts RTCP
	 * packets for this channel.
	 */
	private RtpChannelDatagramFilter rtcpFilter = new OctoDatagramPacketFilter(true /* rtcp */);
	 ...
	 * {@link AbstractEndpoint}s associated with this channel. The audio and video
	 * {@code Octo} channels should have the same set of endpoints, but since the
	 * endpoints of the audio channel a
	 *
	 */
	private final OctoEndpoints octoEndpoints;

	/**
	 * Whether this channel should handle Octo data packets. We always have the Octo
	 * video channel handle data packets, while the Octo audio channel ignores them.
	 */
	 ...
	 *                the list of <tt>Channel</tt>s listed in <tt>content</tt> while
	 *                the new instance is listed there as well.
	 */
	public OctoChannel(Content content, String id) {
		super(content, id, null /* channelBundleId */, OctoTransportManager.NAMESPACE, false /* initiator */);
	...
	public boolean setRtpEncodingParameters(List<SourcePacketExtension> sources,
		...
		if (changed && octoEndpoints != null) {
			octoEndpoints
					.updateEndpoints(Arrays.stream(getStream().getMediaStreamTrackReceiver().getMediaStreamTracks())
							.map(MediaStreamTrackDesc::getOwner).collect(Collectors.toSet()));
	...
	protected void configureStream(MediaStream stream) {
		...
		if (stream != null && stream instanceof AudioMediaStream) {
			((AudioMediaStream) stream).setCsrcAudioLevelListener(new AudioChannelAudioLevelListener(this));
		}
	}
	 ...
	 * RTCP), after if has already {@code receive()}ed a packet from its socket. The
	 * flow of this thread is:
	 * 
	 * <pre>
	...
	/**
	 * Implements a {@link org.ice4j.socket.DatagramPacketFilter} which accepts only
	 * RTP (or only RTCP) packets for this specific {@link OctoChannel}.
	 */
	private class OctoDatagramPacketFilter extends RtpChannelDatagramFilter {
		...
		/**
		 * This filter accepts only packets for its associated {@link OctoChannel}. A
		 * packet is accepted if the conference ID from the Octo header matches the ID
		 * of the channel's conference, and the channel's filter itself accepts it
		 * (based on the configured Payload Type numbers and SSRCs). The second part is
		 * needed because we expect a conference to have two Octo channels -- one for
		 * audio and one for video.
		 * 
		 * @param p the <tt>DatagramPacket</tt> which is assumed to contain an Octo
		 *          packet and is to be accepted (or not) by this filter.
		 *          </p>
		 *          Note that this is meant to work on Octo packets, not RTP packets.
		 * @return {@code true} if the packet should be accepted, and {@code false}
		 *         otherwise.
		 */
		@Override
		public boolean accept(DatagramPacket p) {
			String packetCid = OctoPacket.readConferenceId(p.getData(), p.getOffset(), p.getLength());
			if (!packetCid.equals(conferenceId)) {
				return false;
			}

			MediaType packetMediaType = OctoPacket.readMediaType(p.getData(), p.getOffset(), p.getLength());

			if (mediaType.equals(packetMediaType)) {
				// The RTP/RTCP packet is preceded by the fixed length Octo
				// header.
				boolean packetIsRtcp = RTCPUtils.isRtcp(p.getData(), p.getOffset() + OctoPacket.OCTO_HEADER_LENGTH,
	...
	private void handleDataPacket(DatagramPacket p) {
		...
		System.arraycopy(p.getData(), p.getOffset() + OctoPacket.OCTO_HEADER_LENGTH, msgBytes, 0, msgBytes.length);
	...
	void sendMessage(String msg, String sourceEndpointId) {
		getOctoTransportManager().sendMessage(msg, sourceEndpointId, getConferenceId());
	}
	...
	protected void updatePacketsAndBytes(Conference.Statistics conferenceStatistics) {
		if (conferenceStatistics != null) {
			conferenceStatistics.totalBytesReceivedOcto.addAndGet(statistics.bytesReceived);
			conferenceStatistics.totalBytesSentOcto.addAndGet(statistics.bytesSent);
			conferenceStatistics.totalPacketsReceivedOcto.addAndGet(statistics.packetsReceived);
			conferenceStatistics.totalPacketsSentOcto.addAndGet(statistics.packetsSent);
		}
	}
