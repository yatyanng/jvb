...
public abstract class TransportManager {
	...
	/**
	 * The {@link PortTracker} instance used by jitsi-videobridge to manage dynamic
	 * port allocation.
	 */
	public static final PortTracker portTracker = new PortTracker(DEFAULT_MIN_PORT, DEFAULT_MAX_PORT);

	/**
	 * The <tt>PropertyChangeListener</tt> which listens to changes in the values of
	 * the properties of <tt>Channel</tt>-s in {@link #_channels}. Facilitates
	 * extenders.
	 */
	 ...
	 * @return <tt>true</tt> if the <tt>Channel</tt> was removed, <tt>false</tt> if
	 *         the channel was not in the list.
	 */
	 ...
	 * <tt>ColibriConferenceIQ.ChannelBundle</tt> to the values of the respective
	 * properties of this instance. Thus, the specified <tt>iq</tt> may be thought
	 * of as containing a description of this instance.
	 *
	 ...
	 * properties of this instance. Thus, the specified <tt>pe</tt> may be thought
	 ...
	 * allocated by this instance for the purposes of a specific <tt>Channel</tt>.
	 *
	 * @param channel the <tt>Channel</tt> for which to return the
	 *                <tt>StreamConnector</tt>.
	 * @return the <tt>StreamConnector</tt> which represents the datagram sockets
	...
	/**
	 * Gets the <tt>MediaStreamTarget</tt> which represents the remote addresses to
	 * transmit RTP and RTCP to and from. A non-<tt>null</tt> remote address will
	 * disable latching on the associated component.
	 *
	 * @return the <tt>MediaStreamTarget</tt> which represents the remote addresses
	 *         to transmit RTP and RTCP to and from
	 */
	...
	/**
	 * Starts establishing connectivity between the local endpoint represented by
	 * this instance and the remote endpoint represented by a specific
	 * <tt>IceUdpTransportPacketExtension</tt>. The method should return quickly. If
	 * the connectivity establishment requires a lengthy amount of time to complete,
	 * the method should start executing it asynchronously.
	 *
	 * @param transport an <tt>IceUdpTransportPacketExtension</tt> which specifies
	 *                  connectivity-related information about the remote endpoint
	 */
	public abstract void startConnectivityEstablishment(IceUdpTransportPacketExtension transport);

	/**
	 * Notifies this <tt>TransportManager</tt> that the configured RTP Payload Type
	 * numbers for one of its <tt>RtpChannel</tt>s have been updated.
	 *
	 * @param channel
	 */
	...
	/**
	 * Logs a warning if the addition of <tt>channel</tt> to <tt>_channels</tt> will
	 * result in the same Payload Type number being received by more than one
	 * channel (which is bound to cause issues and probably indicates a problem on
	 * the signalling side).
	 *
	...
	/**
	 * @return the {@link TransportCCEngine} instance, if any, associated with this
	 *         transport channel.
	 */
