...
public class Endpoint extends AbstractEndpoint {
	...
	/**
	 * The name of the <tt>Endpoint</tt> property <tt>selectedEndpoint</tt> which
	 * specifies the JID of the currently selected <tt>Endpoint</tt> of this
	 * <tt>Endpoint</tt>.
	 */
	 ...
	 */
	@Deprecated
	public final static String ENABLE_LIPSYNC_HACK_PNAME = Endpoint.class.getName() + ".ENABLE_LIPSYNC_HACK";
	...
	/**
	 * The set of currently selected <tt>Endpoint</tt>s at this <tt>Endpoint</tt>.
	 */
	 ...
	 * without bundle an endpoint might have multiple channels with different ICE
	 * Agents. In this case one of the channels will be chosen (in an unspecified
	 * way).
	 *
	 * Initialized lazily.
	 */
	 ...
	 *                   <tt>Conference</tt> with which the new instance is to be
	 *                   initialized
	 * @param conference
	 */
	...
	/**
	 * Returns this {@link Endpoint}'s {@link SctpConnection}, if any. Note that
	 * this should NOT be used for sending messages -- use the abstract
	...
	void pinnedEndpointsChanged(Set<String> newPinnedEndpoints) {
		...
		if (!oldPinnedEndpoints.equals(newPinnedEndpoints)) {
			...
			if (logger.isDebugEnabled()) {
				logger.debug(getID() + " pinned " + Arrays.toString(pinnedEndpoints.toArray()));
			}

			firePropertyChange(PINNED_ENDPOINTS_PROPERTY_NAME, oldPinnedEndpoints, pinnedEndpoints);
		}
	}
	...
	void selectedEndpointsChanged(Set<String> newSelectedEndpoints) {
		...
		if (!oldSelectedEndpoints.equals(newSelectedEndpoints)) {
			...
			if (logger.isDebugEnabled()) {
				logger.debug(getID() + " selected " + Arrays.toString(selectedEndpoints.toArray()));
			}

			firePropertyChange(SELECTED_ENDPOINTS_PROPERTY_NAME, oldSelectedEndpoints, selectedEndpoints);
		}
	}

	/**
	 * Sends a specific <tt>String</tt> <tt>msg</tt> over the data channel of this
	 * <tt>Endpoint</tt>.
	 *
	...
	/**
	 * Checks whether a WebSocket connection with a specific password string should
	 * be accepted for this {@link Endpoint}.
	 * 
	 * @param password the
	...
	public boolean acceptWebSocket(String password) {
		...
		if (icePassword == null || !icePassword.equals(password)) {
			logger.warn("Incoming web socket request with an invalid password." + "Expected: " + icePassword
					+ ", received " + password);
			return false;
		}
	...
	/**
	 * @return the password of the ICE Agent associated with this {@link Endpoint}.
	 */
	...
	public void incrementSelectedCount() {
		...
		if (newValue == 1) {
			...
			if (logger.isDebugEnabled()) {
				logger.debug("Endpoint " + getID() + " is now " + "selected, sending message: " + selectedUpdate);
			}
			try {
	...
	public void decrementSelectedCount() {
		...
		if (newValue == 0) {
			...
			if (logger.isDebugEnabled()) {
				logger.debug("Endpoint " + getID() + " is no longer " + "selected, sending message: " + selectedUpdate);
			}
			try {
				sendMessage(selectedUpdate);
			} catch (IOException e) {
