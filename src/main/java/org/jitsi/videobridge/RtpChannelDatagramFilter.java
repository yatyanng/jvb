...
public class RtpChannelDatagramFilter implements DatagramPacketFilter {
	/**
	 * The <tt>Logger</tt> used by the <tt>RtpChannelDatagramFilter</tt> class and
	 ...
	 * @param channel      the channel for which to work.
	 * @param rtcp         whether to accept RTP or RTCP packets.
	 * @param acceptNonRtp {@code true} to accept packets which are neither RTP nor
	 *                     RTCP (e.g. DTLS, STUN, ZRTP); otherwise, {@code false}.
	 */
	...
	/**
	 * Returns <tt>true</tt> if this <tt>RtpChannelDatagramFilter</tt> should accept
	 * an RTCP packet described by <tt>data</tt>, <tt>off</tt>, and <tt>len</tt>.
	...
	/**
	 * Returns <tt>true</tt> if this <tt>RtpChannelDatagramFilter</tt> should accept
	 * an RTP packet with a payload type number <tt>pt</tt>. Checks whether
	 * <tt>pt</tt> is a payload type configured for the <tt>channel</tt>.
	 *
	 * @param pt the payload type number of the packet.
	 * @return <tt>true</tt> if this <tt>RtpChannelDatagramFilter</tt> should accept
	 *         an RTP packet with a payload type number <tt>pt</tt>; otherwise,
	 *         <tt>false</tt>.
	 */
		...
		if (channelPTs == null || channelPTs.length == 0) {
			...
			if (channel.getChannelBundleId() == null) {
				...
				if (!missingPtsWarningLogged) {
					...
					logger.warn("No payload-types specified for channel " + channel.getID() + " while bundle is in use."
							+ " Packets are dropped.");
				}
