...
public class OctoEndpoints {
	...
	/**
	 * Used to synchronize access to the {@link Endpoint}s of the conference while
	 * accessed through {@link OctoEndpoints}. Note that some of the operations
	 * access the {@link Conference}'s list of endpoints which uses a separate lock.
	 * Therefore, to avoid possible deadlocks this class' API MUST NOT be called
	 * while holding a lock on the conference endpoints list.
	 */
	private final Object endpointsSyncRoot = new Object();

	/**
	 * The {@link OctoEndpointMessageTransport} used to parse and handle incoming
	 * data messages from Octo.
	 */
	...
	/**
	 * @return the {@link Conference} associated with this {@link OctoEndpoints}.
	 */
	...
	/**
	 * Gets the list of all Octo endpoints from the associated {@link Conference}.
	 * That is returns all conference endpoints which are instances of
	 * {@link OctoEndpoint}.
	 * 
	 * @return the list of all {@link OctoEndpoint}s in the conference.
	 */
	private List<OctoEndpoint> getOctoEndpoints() {
		return conference.getEndpoints().stream().filter(e -> e instanceof OctoEndpoint).map(e -> (OctoEndpoint) e)
	...
	/**
	 * Updates the list of {@link OctoEndpoint}s maintained by this instance. The
	 * list {@code endpointIds} specifies the current list of endpoints associated
	 * with a particular channel. If the list contains IDs which this instance does
	 * not know about, then an associated endpoint should be added. But if an
	 * {@link OctoEndpoint}'s ID is missing from the list, this does not necessarily
	 * mean that we should expire/remove it (because it might have tracks in the
	 * other channel).
	 */
	void updateEndpoints(Set<String> endpointIds) {
		synchronized (endpointsSyncRoot) {
			List<OctoEndpoint> octoEndpoints = getOctoEndpoints();
			List<String> octoEndpointIds = octoEndpoints.stream().map(AbstractEndpoint::getID)
					.collect(Collectors.toList());
	...
	/**
	 * Creates a new {@link OctoEndpoint} instance and adds it to the conference.
	 * 
	...
	/**
	 * Finds an {@link OctoEndpoint} in this conference which owns a particular SSRC
	 * (i.e. one of its tracks contains the SSRC).
	 * 
	 * @param ssrc the SSRC.
	...
	AbstractEndpoint findEndpoint(long ssrc) {
		synchronized (endpointsSyncRoot) {
			return getOctoEndpoints().stream()
					.filter(e -> e.getMediaStreamTracks().stream().anyMatch(track -> track.matches(ssrc))).findFirst()
					.orElse(null);
		}
	}
