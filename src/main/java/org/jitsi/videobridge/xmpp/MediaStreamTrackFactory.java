...
public class MediaStreamTrackFactory {
	...
	/**
	 * The system property name that for a boolean that's controlling whether or not
	 * to enable temporal scalability filtering for VP8.
	 */
	 ...
	 *
	 * FIXME: hardcoded ugh.. this should be either signaled or somehow included in
	 * the RTP stream.
	 */
	 ...
	 */
	private static final boolean ENABLE_VP9_SVC = cfg.getBoolean(ENABLE_VP9_SVC_PNAME, false);

	/**
	 * A boolean that's controlling whether or not to enable SVC filtering for
	 * scalable video codecs.
	 */
	private static final Boolean ENABLE_SVC = cfg.getBoolean(ENABLE_SVC_PNAME, false);
	 ...
	 * @param primary        the array of the primary SSRCs for the simulcast
	 *                       streams.
	 ...
	 * @param secondarySsrcs a map of primary ssrc -> list of pairs, where each pair
	 *                       has the secondary ssrc as its key, and the type (rtx,
	 *                       etc.) as its value
	 * @return an array that holds the simulcast encodings.
	 */
	private static RTPEncodingDesc[] createRTPEncodings(MediaStreamTrackDesc track, TrackSsrcs primary, int spatialLen,
			int temporalLen, Map<Long, SecondarySsrcs> secondarySsrcs) {
				...
				for (int temporalIdx = 0; temporalIdx < temporalLen; temporalIdx++) {
					...
					if (spatialIdx > 0 && temporalIdx > 0) {
						...
						dependencies = new RTPEncodingDesc[] {
								rtpEncodings[qid(streamIdx, spatialIdx, temporalIdx - 1, spatialLen, temporalLen)],
								rtpEncodings[qid(streamIdx, spatialIdx - 1, temporalIdx, spatialLen, temporalLen)] };
					...
					rtpEncodings[idx] = new RTPEncodingDesc(track, idx, primary.get(streamIdx), temporalId, spatialId,
							height, frameRate, dependencies);
					...
					if (ssrcSecondarySsrcs != null) {
						ssrcSecondarySsrcs.forEach(ssrcSecondarySsrc -> {
							...
							if (type == null) {
								logger.error("Unable to find a mapping for" + " secondary ssrc type "
										+ ssrcSecondarySsrc.type + " will NOT included this secondary ssrc as"
										+ " an encoding");
							} else {
	...
	/**
	 * Get the 'secondary' ssrcs for the given primary ssrc. 'Secondary' here is
	 * defined as things like rtx or fec ssrcs.
	 * 
	 ...
	 */
	private static Map<Long, SecondarySsrcs> getAllSecondarySsrcs(TrackSsrcs ssrcs,
	 ...
	 * @return
	 */
	private static List<SourceGroupPacketExtension> getGroups(final String semantics,
			final List<SourceGroupPacketExtension> groups) {
		return groups.stream().filter(sg -> sg.getSemantics().equalsIgnoreCase(semantics)).collect(Collectors.toList());
	}

	/**
	 * Removes all sources which correspond to an ssrc in trackSsrcs, or any groups
	 * with a primary ssrc in trackSsrcs. NOTE: modifies sources and sourceGroups in
	 * place
	 * 
	 * @param trackSsrcs   the primary ssrcs for which all corresponding ssrcs will
	 *                     be removed from the given sources and groups
	...
	private static void removeReferences(TrackSsrcs trackSsrcs, List<SourcePacketExtension> sources,
		...
		List<SourceGroupPacketExtension> groupsToRemove = sourceGroups.stream()
				.filter(group -> group.getSources().stream().anyMatch(source -> trackSsrcs.contains(source.getSSRC())))
				.collect(Collectors.toList());

		sourceGroups.removeAll(groupsToRemove);

		/*
		 * Remove not only the ssrcs in the track itself, but any ssrcs that were in
		 * groups along with ssrcs from the track. E.g. if we have:` SIM 1 2 3 RTX 1 10
		 * RTX 2 20 RTX 3 30 then we need to make sure ssrcs 10, 20 and 30 don't create
		 * tracks of their own and are removed along with the processing of the track
		 * with ssrcs 1, 2 and 3.
		 */
		Set<Long> ssrcsToRemove = extractSsrcs(groupsToRemove);
		sources.removeIf(source -> trackSsrcs.contains(source.getSSRC()) || ssrcsToRemove.contains(source.getSSRC()));
	}
	...
	/**
	 * Given the sources and groups, return a list of the ssrcs for each unique
	 * track
	 * 
	 * @param sources      the set of sources
	 * @param sourceGroups the set of source groups
	 * @return a list of {@link TrackSsrcs}. Each TrackSsrc object represents a set
	 *         of primary video ssrcs belonging to a single track (video source)
	 */
	private static List<TrackSsrcs> getTrackSsrcs(List<SourcePacketExtension> sources,
		...
		Arrays.asList(SourceGroupPacketExtension.SEMANTICS_SIMULCAST, SourceGroupPacketExtension.SEMANTICS_FID,
				SourceGroupPacketExtension.SEMANTICS_FEC).forEach(groupSem -> {
					...
					groups.forEach(group -> {
						...
						if (groupSem.equalsIgnoreCase(SourceGroupPacketExtension.SEMANTICS_SIMULCAST)) {
							ssrcs = group.getSources().stream().map(SourcePacketExtension::getSSRC)
									.collect(Collectors.toList());
						} else {
		...
		if (!sourceGroupsCopy.isEmpty()) {
			logger.warn("Unprocessed source groups: "
					+ sourceGroupsCopy.stream().map(SourceGroupPacketExtension::toXML).reduce(String::concat));
		}
		...
		sourcesCopy.forEach(source -> {
			if (source.getSSRC() != -1) {
				...
				if (sourcesCopy.size() > 1) {
					logger.warn("Received an empty source, which is " + "a signal to clear all sources, but there were "
	...
	/**
	 * Updates the given list of {@link TrackSsrcs}, setting the {@code owner} field
	 * according to the {@code owner} attribute in {@code ssrc-info} extensions
	 * contained in {@code sources}.
	 * 
	 * @param sources        the list of {@link SourcePacketExtension} which contain
	 *                       the {@code owner} as an attribute of a
	 *                       {@code ssrc-info} child. The list or the objects in the
	...
	private static void setOwners(List<SourcePacketExtension> sources, List<TrackSsrcs> trackSsrcsList) {
		for (TrackSsrcs trackSsrcs : trackSsrcsList) {
			...
			SourcePacketExtension trackSource = sources.stream().filter(source -> source.getSSRC() == primarySsrc)
					.findAny().orElse(null);
	 ...
	 * {@link SourcePacketExtension}. Jicofo includes the full occupant JID of the
	 * endpoint as the owner of a {@link SSRCInfoPacketExtension}, but in
	 * jitsi-videobridge we only care about the resource part, which coincides with
	 * the ID of the Colibri endpoint associated with the owner.
	 *
	 * @param source the {@link SourcePacketExtension} from which to extract the
	 *               owner.
	 * @return the owner/endpoint ID as a {@link String}.
	 */
	public static String getOwner(SourcePacketExtension source) {
		SSRCInfoPacketExtension ssrcInfoPacketExtension = source == null ? null
	 ...
	 *                                 will receive the created
	 *                                 {@link MediaStreamTrackDesc}s.
	 * @param sources                  The {@link List} of
	 *                                 {@link SourcePacketExtension} that describes
	 *                                 the list of jingle sources.
	 * @param sourceGroups             The {@link List} of
	 *                                 {@link SourceGroupPacketExtension} that
	 *                                 describes the list of jingle source groups.
	 * @return the {@link MediaStreamTrackReceiver} that are described in the jingle
	 *         sources and source groups.
	 */
	public static MediaStreamTrackDesc[] createMediaStreamTracks(MediaStreamTrackReceiver mediaStreamTrackReceiver,
			List<SourcePacketExtension> sources, List<SourceGroupPacketExtension> sourceGroups) {
		...
		trackSsrcsList.forEach(trackSsrcs -> {
			...
			MediaStreamTrackDesc track = createTrack(mediaStreamTrackReceiver, trackSsrcs, numSpatialLayersPerStream,
	 ...
	 *
	 * @return the subjective quality index of the flow specified in the arguments.
	 */
	...
	/**
	 * Describes a single secondary ssrc and its group semantics to its primary ssrc
	 * (e.g. FID or FEC-FR)
	 */
	...
	/**
	 * Groups a set of secondary ssrcs for a single primary ssrc (e.g. its rtx and
	 * fec ssrcs)
	 */
	 ...
	 * @param primarySsrcs               the set of primary video ssrcs belonging to
	 *                                   this track
	 * @param numSpatialLayersPerStream  the number of spatial layers per stream for
	 *                                   this track
	 * @param numTemporalLayersPerStream the number of temporal layers per stream
	 *                                   for this track
	 * @param allSecondarySsrcs          a map of primary ssrc -> SecondarySsrcs,
	 *                                   which lists the ssrc and type of all the
	 *                                   secondary ssrcs for a given primary (e.g.
	 *                                   its corresponding rtx and fec ssrcs)
	 * @return the created MediaStreamTrack
	 */
	private static MediaStreamTrackDesc createTrack(MediaStreamTrackReceiver receiver, TrackSsrcs primarySsrcs,
			int numSpatialLayersPerStream, int numTemporalLayersPerStream,
		...
		RTPEncodingDesc[] encodings = createRTPEncodings(track, primarySsrcs, numSpatialLayersPerStream,
				numTemporalLayersPerStream, allSecondarySsrcs);
